...
RETIRED 2026-08-26 — DO NOT RUN. This was a one-shot July repair. Rerunning it
would silently rewrite model files into marts/uncategorized/ (it regenerates the
leadership-PAC and 13F-positions duplicates whose built tables are on the drop
list) and overwrite hand-fixed models with autogenerated passthroughs. The
maintained generator is scripts/gen_mart_models.py, which now fails closed on
the uncategorized bucket. Kept for history only.
"""
import sys
sys.exit("RETIRED 2026-08-26: this one-shot repair script regenerates duplicate "
         "uncategorized marts if rerun. Use scripts/gen_mart_models.py instead. "
