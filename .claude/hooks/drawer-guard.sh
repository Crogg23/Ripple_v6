...
py_or_skip "the junk drawer guard"
INPUT=$(cat)
TARGET=$("$PY" -c "
import json,sys
...
case "$TARGET" in
  *_JUNK_DRAWER*)
    "$PY" -c "import json; print(json.dumps({'hookSpecificOutput':{'hookEventName':'PreToolUse','permissionDecision':'ask','permissionDecisionReason':'This is the junk drawer — retired attempts, reference only. Never build from it. Still want to open it?'}}))"
    ;;
esac
exit 0
