{{ config(materialized='view', schema='TIMELINE', alias='OPEN_DATA__INTL_CL_DATOSGOB') }}

-- GENERATED by scripts/census/gen_time_views.py -- do not hand-edit.
-- Regenerate after any change to reports/time_index/clock_index.csv.
--
-- Canonical clock for OPEN_DATA.OPEN_DATA__INTL_CL_DATOSGOB.
--   column : METADATA_CREATED   (TEXT)
--   means  : reported -- Uncast TEXT CKAN record-creation stamp — when the dataset was registered on datos.gob.cl.
--   grain  : day
--
-- reported cannot be in the future -- if this row's value is, ripple_clock
-- reads 'planned' instead, not 'reported'. See ripple_row_clock in
-- macros/ripple_time.sql.
--
-- The original columns pass through untouched; the canonical four sit in front
-- of them. Rule 8 of the datetime standard: the raw column is never overwritten,
-- so a mis-parse is always recoverable.

select
    {{ ripple_ts_from_date('src."METADATA_CREATED"') }} as ripple_ts,
    {{ ripple_grain('day') }} as ripple_grain,
    {{ ripple_row_clock(ripple_ts_from_date('src."METADATA_CREATED"'), 'reported') }} as ripple_clock,
    'OPEN_DATA.OPEN_DATA__INTL_CL_DATOSGOB'::varchar as ripple_source,
    src.*
from {{ ref('open_data__intl_cl_datosgob') }} as src
