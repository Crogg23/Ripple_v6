{{ config(materialized='view', schema='TIMELINE', alias='HEALTH__FED_CMS_HOSPITAL_ENROLLMENTS') }}
...
--
-- Canonical clock for HEALTH.HEALTH__FED_CMS_HOSPITAL_ENROLLMENTS.
--   column : INCORPORATION_DATE   (DATE)
--   means  : happened -- try_to_date of incorporation; 60 epoch-1970 rows and an 1800-01-01 floor.
--   grain  : day
--
...
select
    ...
    'HEALTH.HEALTH__FED_CMS_HOSPITAL_ENROLLMENTS'::varchar as ripple_source,
    src.*
from {{ ref('health__fed_cms_hospital_enrollments') }} as src
