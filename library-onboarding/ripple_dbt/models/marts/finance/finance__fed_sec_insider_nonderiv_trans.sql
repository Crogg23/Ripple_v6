...
select
    ...
    {{ stg_float('trans_shares') }}                      as shares,
    {{ stg_float('trans_pricepershare') }}               as price_per_share,
    {{ stg_float('trans_shares') }} * {{ stg_float('trans_pricepershare') }} as transaction_value,
    trim(trans_acquired_disp_cd)                     as acquired_disposed,
    {{ stg_float('shrs_ownd_folwng_trans') }}            as shares_owned_after,
    {{ stg_float('valu_ownd_folwng_trans') }}            as value_owned_after,
