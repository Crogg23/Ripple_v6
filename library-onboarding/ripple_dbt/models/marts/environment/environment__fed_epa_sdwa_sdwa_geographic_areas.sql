...
-- 2026-09-10: COUNTY_FIPS added. COUNTY_SERVED is a bare name and STATE_SERVED is blank on
...
county_dim as (
    select
        ...
        upper(regexp_replace(regexp_replace(COUNTYNAME,
            '[[:space:]]+(County|Parish|Borough|Census Area|Municipio|City and Borough|Municipality|city)$', '', 1, 0, 'i'),
...
left join county_dim d
   ...
   and d.name_key = upper(regexp_replace(regexp_replace(s.COUNTY_SERVED,
        '[[:space:]]+(County|Parish|Borough|Census Area|Municipio|City and Borough|Municipality|city)$', '', 1, 0, 'i'),
        '[^A-Za-z0-9]', ''))
