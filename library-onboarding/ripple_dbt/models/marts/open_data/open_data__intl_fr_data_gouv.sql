...
select
    ...
    try_to_timestamp_ntz(left(last_modified, 19), 'YYYY-MM-DD"T"HH24:MI:SS')
                                                   as last_modified,
    archived,
    {{ stg_int('nb_resources') }}                    as nb_resources,
    resources_formats,
    {{ stg_int('nb_reuses') }}                       as nb_reuses,
    {{ stg_int('nb_followers') }}                    as nb_followers,
    {{ stg_int('nb_views') }}                        as nb_views,
    {{ stg_int('nb_resource_downloads') }}           as nb_resource_downloads,
    {{ stg_float('quality_score') }}                   as quality_score,
    tags,
    spatial_granularity,
    spatial_zones,
