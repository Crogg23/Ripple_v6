...
renamed as (
    ...
    select
        ...
        {{ stg_float('trim(NAMEPLATE_POWER_FACTOR)') }}                  as nameplate_power_factor,
        {{ stg_float('trim(SUMMER_CAPACITY_MW)') }}                      as summer_capacity_mw,
        {{ stg_float('trim(WINTER_CAPACITY_MW)') }}                      as winter_capacity_mw,
        {{ stg_float('trim(MINIMUM_LOAD_MW)') }}                         as minimum_load_mw,
        {{ stg_int('trim(MONTH_UPRATE_OR_DERATE_COMPLETED)') }}        as month_uprate_or_derate_completed,
        {{ stg_int('trim(YEAR_UPRATE_OR_DERATE_COMPLETED)') }}         as year_uprate_or_derate_completed,
        {{ stg_int('trim(OPERATING_MONTH)') }}                         as operating_month,
        {{ stg_int('trim(OPERATING_YEAR)') }}                          as operating_year,
        {{ stg_int('trim(PLANNED_RETIREMENT_MONTH)') }}                as planned_retirement_month,
        {{ stg_int('trim(PLANNED_RETIREMENT_YEAR)') }}                 as planned_retirement_year,
        {{ stg_int('trim(TURBINES_OR_HYDROKINETIC_BUOYS)') }}          as turbines_or_hydrokinetic_buoys,
        {{ stg_float('trim(PLANNED_NET_SUMMER_CAPACITY_UPRATE_MW)') }}   as planned_net_summer_capacity_uprate_mw,
        {{ stg_float('trim(PLANNED_NET_WINTER_CAPACITY_UPRATE_MW)') }}   as planned_net_winter_capacity_uprate_mw,
        {{ stg_int('trim(PLANNED_UPRATE_MONTH)') }}                    as planned_uprate_month,
        {{ stg_int('trim(PLANNED_UPRATE_YEAR)') }}                     as planned_uprate_year,
        {{ stg_float('trim(PLANNED_NET_SUMMER_CAPACITY_DERATE_MW)') }}   as planned_net_summer_capacity_derate_mw,
        {{ stg_float('trim(PLANNED_NET_WINTER_CAPACITY_DERATE_MW)') }}   as planned_net_winter_capacity_derate_mw,
        {{ stg_int('trim(PLANNED_DERATE_MONTH)') }}                    as planned_derate_month,
        {{ stg_int('trim(PLANNED_DERATE_YEAR)') }}                     as planned_derate_year,
        {{ stg_float('trim(PLANNED_NEW_NAMEPLATE_CAPACITY_MW)') }}       as planned_new_nameplate_capacity_mw,
        {{ stg_int('trim(PLANNED_REPOWER_MONTH)') }}                   as planned_repower_month,
        {{ stg_int('trim(PLANNED_REPOWER_YEAR)') }}                    as planned_repower_year,
        {{ stg_int('trim(OTHER_MODIFICATIONS_MONTH)') }}               as other_modifications_month,
        {{ stg_int('trim(OTHER_MODIFICATIONS_YEAR)') }}                as other_modifications_year,
