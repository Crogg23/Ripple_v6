...
renamed as (
    ...
    select
        ...
        {{ stg_date("nullif(trim(DKTDATE), '01/01/1900')") }}                     as docket_date,
        ...
        {{ stg_date("nullif(trim(DDKTDATE), '01/01/1900')") }}                    as district_docket_date,
        {{ stg_date("nullif(trim(APPDATE), '01/01/1900')") }}                     as appeal_date,
        {{ stg_date("nullif(trim(TRANSDATE), '01/01/1900')") }}                   as transfer_date,
        ...
        -- milestones
        {{ stg_date("nullif(trim(CRECDATE), '01/01/1900')") }}                    as court_record_date,
        nullif(trim(BRFILED), '01/01/1900')            as briefs_filed,
        {{ stg_date("nullif(trim(SUBDATE), '01/01/1900')") }}                     as submission_date,
        {{ stg_date("nullif(trim(HEARDATE), '01/01/1900')") }}                    as hearing_date,
        {{ stg_date("nullif(trim(JUDGDATE), '01/01/1900')") }}                    as judgment_date,
