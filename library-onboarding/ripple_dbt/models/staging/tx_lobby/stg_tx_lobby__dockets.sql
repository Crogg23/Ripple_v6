...
/*
  ...
  Texas lobby docket/proceeding designations: one row per docket line (lobbydocketdesigid unique).
  Grain: one row = one docket line.
*/

with source as (
    select * from {{ source('ripple_raw', 'TX_LOBBY_DOCKETS') }}
),
...
renamed as (
    select
        ...
        nullif(trim(LOBBYDOCKETDESIGID), '')                           as docket_id,
        nullif(trim(DESIGNATIONTEXT), '')                              as designationtext,
        nullif(trim(AGENCYNAME), '')                                   as agencyname,
