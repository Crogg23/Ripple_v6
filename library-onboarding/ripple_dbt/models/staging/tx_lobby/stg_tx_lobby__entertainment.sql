...
/*
  ...
  Texas lobby entertainment expenditures: one row per activity (lobbyactivityid unique).
  Grain: one row = one entertainment activity.
*/

with source as (
    select * from {{ source('ripple_raw', 'TX_LOBBY_ENTERTAINMENT') }}
),
...
renamed as (
    select
        ...
        nullif(trim(ENTERTAINMENTNAME), '')                            as entertainmentname,
        nullif(trim(ENTERTAINMENTSTREETADDR1), '')                     as entertainmentstreetaddr1,
        nullif(trim(ENTERTAINMENTSTREETADDR2), '')                     as entertainmentstreetaddr2,
        nullif(trim(ENTERTAINMENTSTREETCITY), '')                      as entertainmentstreetcity,
        nullif(trim(ENTERTAINMENTSTREETSTATECD), '')                   as entertainmentstreetstatecd,
        nullif(trim(ENTERTAINMENTSTREETCOUNTRYCD), '')                 as entertainmentstreetcountrycd,
        nullif(trim(ENTERTAINMENTSTREETPOSTALCODE), '')                as entertainmentstreetpostalcode,
        nullif(trim(ENTERTAINMENTSTREETREGION), '')                    as entertainmentstreetregion,
