...
renamed as (
    ...
    select
        ...
        {{ stg_int('trim(DEMANDED)') }}                  as amount_demanded,
        ...
        -- transfer
        {{ stg_date("nullif(trim(TRANSDAT), '01/01/1900')") }}                    as transfer_date,
        ...
        {{ stg_int('trim(AMTREC)') }}                    as amount_received,
        trim(JUDGMENT)                                 as judgment,

        -- milestones
        {{ stg_date("nullif(trim(DJOINED), '01/01/1900')") }}                     as issue_joined_date,
        {{ stg_date("nullif(trim(PRETRIAL), '01/01/1900')") }}                    as pretrial_date,
        {{ stg_date("nullif(trim(TRIBEGAN), '01/01/1900')") }}                    as trial_began_date,
        {{ stg_date("nullif(trim(TRIALEND), '01/01/1900')") }}                    as trial_end_date,
