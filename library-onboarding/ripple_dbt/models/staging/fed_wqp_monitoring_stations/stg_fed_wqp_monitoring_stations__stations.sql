...
renamed as (
    ...
    select
        ...
        {{ stg_float('trim(DRAINAGEAREAMEASURE_MEASUREVALUE)') }} as drainage_area_measure_value,
        trim(DRAINAGEAREAMEASURE_MEASUREUNITCODE)      as drainage_area_measure_unit_code,
        {{ stg_float('trim(CONTRIBUTINGDRAINAGEAREAMEASURE_MEASUREVALUE)') }} as contributing_drainage_area_measure_value,
        trim(CONTRIBUTINGDRAINAGEAREAMEASURE_MEASUREUNITCODE) as contributing_drainage_area_measure_unit_code,
        {{ stg_float('trim(LATITUDEMEASURE)') }}           as latitude,
        {{ stg_float('trim(LONGITUDEMEASURE)') }}          as longitude,
        {{ stg_int('trim(SOURCEMAPSCALENUMERIC)') }}     as source_map_scale_numeric,
        {{ stg_float('trim(HORIZONTALACCURACYMEASURE_MEASUREVALUE)') }} as horizontal_accuracy_measure_value,
        ...
        {{ stg_float('trim(VERTICALMEASURE_MEASUREVALUE)') }} as vertical_measure_value,
        trim(VERTICALMEASURE_MEASUREUNITCODE)          as vertical_measure_unit_code,
        {{ stg_float('trim(VERTICALACCURACYMEASURE_MEASUREVALUE)') }} as vertical_accuracy_measure_value,
        ...
        {{ stg_float('trim(WELLDEPTHMEASURE_MEASUREVALUE)') }} as well_depth_measure_value,
        trim(WELLDEPTHMEASURE_MEASUREUNITCODE)         as well_depth_measure_unit_code,
        {{ stg_float('trim(WELLHOLEDEPTHMEASURE_MEASUREVALUE)') }} as well_hole_depth_measure_value,
