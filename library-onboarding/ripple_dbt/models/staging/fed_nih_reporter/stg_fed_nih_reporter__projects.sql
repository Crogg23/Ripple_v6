...
renamed as (
    select
        ...
        {{ stg_int("nullif(trim(FISCAL_YEAR), '')") }}       as fiscal_year,
        ...
        {{ stg_date("left(nullif(trim(PROJECT_START_DATE), ''), 10)") }} as project_start_date,
        {{ stg_date("left(nullif(trim(PROJECT_END_DATE), ''), 10)") }}   as project_end_date,
        {{ stg_date("left(nullif(trim(BUDGET_START_DATE), ''), 10)") }}  as budget_start_date,
        {{ stg_date("left(nullif(trim(BUDGET_END_DATE), ''), 10)") }}    as budget_end_date,
        {{ stg_date("left(nullif(trim(AWARD_NOTICE_DATE), ''), 10)") }}  as award_notice_date,
        {{ stg_ts("nullif(trim(DATE_ADDED), '')") }}          as date_added,
