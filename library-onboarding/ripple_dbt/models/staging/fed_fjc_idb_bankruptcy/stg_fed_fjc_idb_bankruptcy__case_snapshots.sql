...
renamed as (
    ...
    select
        ...
        {{ stg_date("nullif(trim(ORGFLDT), '01/01/1900')") }}                     as original_filing_date,
        ...
        {{ stg_date("nullif(trim(D1CHGDT), '01/01/1900')") }}                     as debtor1_change_date,
        ...
        {{ stg_date("nullif(trim(D2CHGDT), '01/01/1900')") }}                     as debtor2_change_date,
        ...
        {{ stg_float('trim(TOTASSTS)') }}                  as total_assets,
        {{ stg_float('trim(REALPROP)') }}                  as real_property,
        {{ stg_float('trim(PERSPROP)') }}                  as personal_property,
        {{ stg_float('trim(TOTLBLTS)') }}                  as total_liabilities,
        {{ stg_float('trim(SECURED)') }}                   as secured_claims,
        {{ stg_float('trim(UNSECPR)') }}                   as unsecured_priority_claims,
        {{ stg_float('trim(UNSECNPR)') }}                  as unsecured_nonpriority_claims,
        {{ stg_float('trim(DSCHRGD)') }}                   as debt_discharged,
        {{ stg_float('trim(NDSCHRGD)') }}                  as debt_not_discharged,
        {{ stg_float('trim(TOTDBT)') }}                    as total_debt,
        trim(CNTMNTHI)                                 as count_monthly_income,
        {{ stg_float('trim(AVGMNTHI)') }}                  as avg_monthly_income,
        {{ stg_float('trim(AVGMNTHE)') }}                  as avg_monthly_expenses,
        ...
        -- closing / disposition
        {{ stg_date("nullif(trim(CLOSEDT), '01/01/1900')") }}                     as close_date,
        ...
        {{ stg_date("nullif(trim(D1FDSPDT), '01/01/1900')") }}                    as debtor1_final_disposition_date,
        ...
        {{ stg_date("nullif(trim(D2FDSPDT), '01/01/1900')") }}                    as debtor2_final_disposition_date,
