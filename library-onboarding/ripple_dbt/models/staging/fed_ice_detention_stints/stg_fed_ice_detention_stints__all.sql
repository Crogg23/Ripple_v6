...
renamed as (
    select
        ...
        {{ stg_ts_tz('trim(STAY_BOOK_IN_DATE_TIME)') }}::timestamp_ntz  as stay_book_in_at,
        {{ stg_ts_tz('trim(BOOK_IN_DATE_TIME)') }}::timestamp_ntz       as book_in_at,
        {{ stg_ts_tz('trim(BOOK_OUT_DATE_TIME)') }}::timestamp_ntz      as book_out_at,
        {{ stg_ts_tz('trim(STAY_BOOK_OUT_DATE_TIME)') }}::timestamp_ntz as stay_book_out_at,
        {{ stg_date('trim(STAY_BOOK_OUT_DATE)') }}                     as stay_book_out_date,
        ...
        {{ stg_int('trim(BIRTH_YEAR)') }}                           as birth_year,
        ...
        {{ stg_date('trim(ENTRY_DATE)') }}                             as entry_date,
        ...
        {{ stg_date('trim(MSC_CHARGE_DATE)') }}                        as msc_charge_date,
        {{ stg_date('trim(MSC_CONVICTION_DATE)') }}                    as msc_conviction_date,
        {{ stg_int('trim(MSC_SENTENCE_DAYS)') }}                    as msc_sentence_days,
        {{ stg_int('trim(MSC_SENTENCE_MONTHS)') }}                  as msc_sentence_months,
        {{ stg_int('trim(MSC_SENTENCE_YEARS)') }}                   as msc_sentence_years,
        ...
        {{ stg_date('trim(FINAL_ORDER_DATE)') }}                       as final_order_date,
        nullif(trim(FINAL_CHARGE), '')                            as final_charge,
        nullif(trim(FINAL_PROGRAM), '')                           as final_program,
        {{ stg_date('trim(DEPARTED_DATE)') }}                          as departed_date,
        nullif(trim(DEPARTURE_COUNTRY), '')                       as departure_country,
        {{ stg_int('trim(INITIAL_BOND_SET_AMOUNT)') }}              as initial_bond_set_amount,
        {{ stg_date('trim(INITIAL_BOND_SET_DATE)') }}                  as initial_bond_set_date,
        {{ stg_int('trim(BOND_POSTED_AMOUNT)') }}                   as bond_posted_amount,
        {{ stg_date('trim(BOND_POSTED_DATE)') }}                       as bond_posted_date,
        {{ stg_int('trim(INITIAL_BOND_SET_AMOUNT_LOWEST_SEEN)') }}  as initial_bond_set_amount_lowest_seen,
        {{ stg_date('trim(INITIAL_BOND_SET_DATE_EARLIEST_SEEN)') }}    as initial_bond_set_date_earliest_seen,
        {{ stg_int('trim(BOND_POSTED_AMOUNT_LOWEST_SEEN)') }}       as bond_posted_amount_lowest_seen,
        {{ stg_date('trim(BOND_POSTED_DATE_EARLIEST_SEEN)') }}         as bond_posted_date_earliest_seen,
