...
renamed as (
    ...
    select
        ...
        {{ stg_date('trim(APPLIEDDATE)') }}                  as applied_date,
        {{ stg_date('trim(DECLARATIONDATE)') }}              as declaration_date,
        ...
        {{ stg_int('trim(OCCUPANTSUNDERTWO)') }}          as occupants_under_two,
        {{ stg_int('trim(OCCUPANTS2TO5)') }}              as occupants_2_to_5,
        {{ stg_int('trim(OCCUPANTS6TO18)') }}             as occupants_6_to_18,
        {{ stg_int('trim(OCCUPANTS19TO64)') }}            as occupants_19_to_64,
        {{ stg_int('trim(OCCUPANTS65ANDOVER)') }}         as occupants_65_and_over,
        {{ stg_int('trim(GROSSINCOME)') }}                as gross_income,
        ...
        {{ stg_float('trim(FLOODDAMAGEAMOUNT)') }}          as flood_damage_amount,
        trim(FOUNDATIONDAMAGE)                          as foundation_damage,
        {{ stg_float('trim(FOUNDATIONDAMAGEAMOUNT)') }}     as foundation_damage_amount,
        trim(ROOFDAMAGE)                                as roof_damage,
        {{ stg_float('trim(ROOFDAMAGEAMOUNT)') }}           as roof_damage_amount,
        {{ stg_int('trim(WATERLEVEL)') }}                 as water_level,
        ...
        {{ stg_float('trim(PPFVL)') }}                      as ppfvl,
        {{ stg_float('trim(RPFVL)') }}                      as rpfvl,
        ...
        {{ stg_float('trim(IHPAMOUNT)') }}                  as ihp_amount,
        {{ stg_int('trim(IHPMAX)') }}                     as ihp_max,
        trim(IHPREFERRAL)                               as ihp_referral,

        -- HA (Housing Assistance)
        trim(HAELIGIBLE)                                as ha_eligible,
        {{ stg_float('trim(HAAMOUNT)') }}                   as ha_amount,
        {{ stg_int('trim(HAMAX)') }}                      as ha_max,
        trim(HAREFERRAL)                                as ha_referral,
        trim(HASTATUS)                                  as ha_status,

        -- FIP (Flood Insurance Purchase)
        {{ stg_int('trim(FIPAMOUNT)') }}                  as fip_amount,

        -- repair / replacement / rental assistance
        trim(REPAIRASSISTANCEELIGIBLE)                  as repair_assistance_eligible,
        {{ stg_float('trim(REPAIRAMOUNT)') }}               as repair_amount,
        trim(REPLACEMENTASSISTANCEELIGIBLE)             as replacement_assistance_eligible,
        {{ stg_float('trim(REPLACEMENTAMOUNT)') }}          as replacement_amount,
        trim(RENTALASSISTANCEELIGIBLE)                  as rental_assistance_eligible,
        {{ stg_float('trim(RENTALASSISTANCEAMOUNT)') }}     as rental_assistance_amount,
        {{ stg_date('trim(RENTALASSISTANCEENDDATE)') }}      as rental_assistance_end_date,
        ...
        {{ stg_float('trim(ONAAMOUNT)') }}                  as ona_amount,
        {{ stg_int('trim(ONAMAX)') }}                     as ona_max,
        trim(ONAREFERRAL)                               as ona_referral,
        trim(ONADENTALASSISTELIGIBLE)                   as ona_dental_assist_eligible,
        {{ stg_float('trim(ONADENTALASSISTAMOUNT)') }}      as ona_dental_assist_amount,
        trim(ONAFUNERALASSISTELIGIBLE)                  as ona_funeral_assist_eligible,
        {{ stg_float('trim(ONAFUNERALASSISTAMOUNT)') }}     as ona_funeral_assist_amount,
        trim(ONAMEDICALASSISTELIGIBLE)                  as ona_medical_assist_eligible,
        {{ stg_float('trim(ONAMEDICALASSISTAMOUNT)') }}     as ona_medical_assist_amount,
        trim(ONAMOVINGASSISTELIGIBLE)                   as ona_moving_assist_eligible,
        {{ stg_float('trim(ONAMOVINGASSISTAMOUNT)') }}      as ona_moving_assist_amount,
        trim(ONAOTHERASSISTELIGIBLE)                    as ona_other_assist_eligible,
        {{ stg_float('trim(ONAOTHERASSISTAMOUNT)') }}       as ona_other_assist_amount,

        -- personal property / unmet need
        trim(PERSONALPROPERTYELIGIBLE)                  as personal_property_eligible,
        {{ stg_float('trim(PERSONALPROPERTYAMOUNT)') }}     as personal_property_amount,
        {{ stg_float('trim(UNMETNEEDPP)') }}                as unmet_need_pp,
        {{ stg_float('trim(UNMETNEEDRP)') }}                as unmet_need_rp,

        -- transient accommodation / TSA / SBA
        trim(TRANSIENTACCOMMODELIGIBLE)                 as transient_accommod_eligible,
        {{ stg_float('trim(TRANSIENTACCOMMODAMOUNT)') }}    as transient_accommod_amount,
