...
-- (CANDID, COMMITTEE, FILING, REFNO) is NEAR-unique (193,493 distinct of
-- 193,741 rows -- 248 collisions, verified live by the orchestrator), so a
-- row_number() over the full-row hash is appended as a deterministic
-- tiebreaker to make contribution_record_id fully unique (same idiom as the
-- FJC IDB staging models).
...
with source as (
    ...
    select * from {{ source('ripple_raw', 'ST_NYC_CFB_CAMPAIGN_2001_CONTRIBUTION') }}
...
keyed as (
    ...
    select
        source.*,
        {{ dbt_utils.generate_surrogate_key(['CANDID', 'COMMITTEE', 'FILING', 'REFNO']) }}
            || '-'
            || row_number() over (
                   partition by CANDID, COMMITTEE, FILING, REFNO
                   order by hash(*)
...
renamed as (
    ...
    select
        ...
        trim(CANDID)                                   as candidate_id,
        trim(CANCLASS)                                 as candidate_class,
        trim(CANDLAST)                                 as candidate_last_name,
        trim(CANDFIRST)                                as candidate_first_name,
        trim(CANDMI)                                   as candidate_middle_initial,
        ...
        {{ stg_date('trim(DATE)') }}                        as contribution_date,
        {{ stg_date('trim(REFUNDDATE)') }}                  as refund_date,
