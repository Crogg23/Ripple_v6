...
renamed as (
    ...
    select
        ...
        {{ stg_int('trim(TRACS_OVERALL_EXP_FISCAL_YEAR)') }}                       as tracs_overall_exp_fiscal_year,
        ...
        {{ stg_int('trim(CONTRACT_TERM_MONTHS_QTY)') }}    as contract_term_months_qty,
        ...
        -- units / rents
        {{ stg_int('trim(ASSISTED_UNITS_COUNT)') }}        as assisted_units_count,
        try_to_number(trim(RENT_TO_FMR_RATIO), 20, 8)    as rent_to_fmr_ratio,
        trim(RENT_TO_FMR_DESCRIPTION)                    as rent_to_fmr_description,
        {{ stg_int('trim(C_0BR_COUNT)') }}                 as units_0br_count,
        {{ stg_int('trim(C_1BR_COUNT)') }}                 as units_1br_count,
        {{ stg_int('trim(C_2BR_COUNT)') }}                 as units_2br_count,
        {{ stg_int('trim(C_3BR_COUNT)') }}                 as units_3br_count,
        {{ stg_int('trim(C_4BR_COUNT)') }}                 as units_4br_count,
        {{ stg_int('trim(C_5PLUSBR_COUNT)') }}             as units_5plusbr_count,
        {{ stg_int('trim(C_0BR_FMR)') }}                   as fmr_0br,
        {{ stg_int('trim(C_1BR_FMR)') }}                   as fmr_1br,
        {{ stg_int('trim(C_2BR_FMR)') }}                   as fmr_2br,
        {{ stg_int('trim(C_3BR_FMR)') }}                   as fmr_3br,
        {{ stg_int('trim(C_4BR_FMR)') }}                   as fmr_4br,
