...
with source as (
    ...
    from {{ source('ripple_raw', 'FED_HHS_TAGGS') }}
...
deduped as (
    ...
    -- 2026-09-19: the old partition was award_number. It hid 44 of 45 landing
    -- rows on a single load -- award_number does not identify a row. The dedupe
    -- is now the whole-raw-row hash, so only exact copies are dropped (9 exact
    -- copies in landing).
    select *,
        row_number() over (
