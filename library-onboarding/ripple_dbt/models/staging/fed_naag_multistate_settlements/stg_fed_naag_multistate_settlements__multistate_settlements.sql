...
renamed as (
    select
        ...
        {{ stg_int("nullif(trim(YEAR), '')") }}                  as year,
        ...
        {{ stg_float("nullif(trim(TOTALSETTLEMENTAMOUNT), '')") }} as total_settlement_amount,
        {{ stg_float("nullif(trim(TOTALSTATESHARE), '')") }}       as total_state_share,
        {{ stg_float("nullif(trim(TOTAL_FEDERAL_SHARE), '')") }}   as total_federal_share,
