...
renamed as (
    select
        ...
        {{ stg_int("replace(trim(\"INCIDENT_YEAR\"), ',', '')") }} as incident_year,
        ...
        {{ stg_int("replace(trim(\"AGE_OF_PERSON\"), ',', '')") }} as age_of_person,
        {{ stg_int("replace(trim(\"POSITIVE_ALCOHOL_TESTS\"), ',', '')") }} as positive_alcohol_tests,
        {{ stg_int("replace(trim(\"POSITIVE_DRUG_TESTS\"), ',', '')") }} as positive_drug_tests,
        ...
        {{ stg_int("replace(trim(\"DAYS_AWAY_FROM_WORK\"), ',', '')") }} as days_away_from_work,
        {{ stg_int("replace(trim(\"DAYS_RESTRICTED_ACTIVITY\"), ',', '')") }} as days_restricted_activity,
        ...
        {{ stg_float("replace(trim(\"LATITUDE\"), ',', '')") }} as latitude,
        {{ stg_float("replace(trim(\"LONGITUDE\"), ',', '')") }} as longitude,
