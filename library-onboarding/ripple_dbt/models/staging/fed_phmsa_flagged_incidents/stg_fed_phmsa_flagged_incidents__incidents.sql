...
renamed as (
    ...
    select
        ...
        -- dates
        {{ stg_date('trim(REPORT_RECEIVED_DATE)') }}        as report_received_date,
        {{ stg_int('trim(IYEAR)') }}                     as incident_year,
        ...
        -- location
        {{ stg_float('trim(LOCATION_LATITUDE)') }}         as location_latitude,
        {{ stg_float('trim(LOCATION_LONGITUDE)') }}        as location_longitude,

        -- commodity and release
        trim(COMMODITY_RELEASED_TYPE)                  as commodity_released_type,
        trim(COMMODITY_DETAILS)                        as commodity_details,
        {{ stg_float('trim(UNINTENTIONAL_RELEASE)') }}     as unintentional_release_volume,
        {{ stg_float('trim(INTENTIONAL_RELEASE)') }}       as intentional_release_volume,

        -- fatalities
        trim(FATALITY_IND)                             as fatality_ind,
        {{ stg_int('trim(NUM_EMP_FATALITIES)') }}        as num_employee_fatalities,
        {{ stg_int('trim(NUM_CONTR_FATALITIES)') }}      as num_contractor_fatalities,
        {{ stg_int('trim(NUM_ER_FATALITIES)') }}         as num_emergency_responder_fatalities,
        {{ stg_int('trim(NUM_WORKER_FATALITIES)') }}     as num_worker_fatalities,
        {{ stg_int('trim(NUM_GP_FATALITIES)') }}         as num_public_fatalities,
        {{ stg_int('trim(FATAL)') }}                     as total_fatalities,

        -- injuries
        trim(INJURY_IND)                               as injury_ind,
        {{ stg_int('trim(NUM_EMP_INJURIES)') }}          as num_employee_injuries,
        {{ stg_int('trim(NUM_CONTR_INJURIES)') }}        as num_contractor_injuries,
        {{ stg_int('trim(NUM_ER_INJURIES)') }}           as num_emergency_responder_injuries,
        {{ stg_int('trim(NUM_WORKER_INJURIES)') }}       as num_worker_injuries,
        {{ stg_int('trim(NUM_GP_INJURIES)') }}           as num_public_injuries,
        {{ stg_int('trim(INJURE)') }}                    as total_injuries,
