{{ config(materialized='view', schema='LANDING_CLEAN', alias='FED_SENATE_LDA_FILINGS') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.FED_SENATE_LDA_FILINGS, from the
...
select
    source.*,
    {{ landing_parse_date('"DT_POSTED"', 'iso', 'TEXT') }} as dt_posted_clean_ts,
...
from {{ source('ripple_raw', 'FED_SENATE_LDA_FILINGS') }} as source
