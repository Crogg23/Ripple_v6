{{ config(materialized='view', schema='LANDING_CLEAN', alias='FED_CDC_DATA_PORTAL') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.FED_CDC_DATA_PORTAL, from the
...
select
    source.*,
    {{ landing_parse_date('"UPDATED_AT"', 'epochs', 'TEXT') }} as updated_at_clean_ts,
    {{ landing_parse_audit_epoch('"_INGESTED_AT"') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'FED_CDC_DATA_PORTAL') }} as source
