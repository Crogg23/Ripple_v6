{{ config(materialized='view', schema='LANDING_CLEAN', alias='FED_CMS_OPEN_PAYMENTS_2022') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.FED_CMS_OPEN_PAYMENTS_2022, from the
...
select
    source.*,
    {{ landing_parse_date('"DATE_OF_PAYMENT"', 'us', 'TEXT') }} as date_of_payment_clean_ts,
    {{ landing_parse_date('"PAYMENT_PUBLICATION_DATE"', 'us', 'TEXT') }} as payment_publication_date_clean_ts,
    {{ landing_parse_audit_epoch('"_INGESTED_AT"') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'FED_CMS_OPEN_PAYMENTS_2022') }} as source
