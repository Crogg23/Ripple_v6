{{ config(materialized='view', schema='LANDING_CLEAN', alias='FED_NAAG_MULTISTATE_SETTLEMENTS') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.FED_NAAG_MULTISTATE_SETTLEMENTS, from the
...
select
    source.*,
    {{ landing_parse_date('"DATERESOLVED"', 'us', 'TEXT') }} as dateresolved_clean_ts,
    {{ landing_parse_date('"DATE_ENTRY_CREATED"', 'us', 'TEXT') }} as date_entry_created_clean_ts,
    {{ landing_parse_audit_epoch('"INGESTED_AT"') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'FED_NAAG_MULTISTATE_SETTLEMENTS') }} as source
