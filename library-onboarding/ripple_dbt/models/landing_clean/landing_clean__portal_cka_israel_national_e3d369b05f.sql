{{ config(materialized='view', schema='LANDING_CLEAN', alias='PORTAL_CKA_ISRAEL_NATIONAL_E3D369B05F') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.PORTAL_CKA_ISRAEL_NATIONAL_E3D369B05F, from the
...
select
    source.*,
    {{ landing_parse_date('"DATE_OPEN"', 'iso', 'TEXT') }} as date_open_clean_ts,
    {{ landing_parse_date('"DATE_CLOSE"', 'iso', 'TEXT') }} as date_close_clean_ts,
    {{ landing_parse_date('"INGESTED_AT"', 'iso', 'TEXT') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'PORTAL_CKA_ISRAEL_NATIONAL_E3D369B05F') }} as source
