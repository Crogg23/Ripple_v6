{{ config(materialized='view', schema='LANDING_CLEAN', alias='FED_FDA_DRUG_MASTER_FILES') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.FED_FDA_DRUG_MASTER_FILES, from the
...
select
    source.*,
    {{ landing_parse_date('"SUBMIT_DATE"', 'iso', 'TEXT') }} as submit_date_clean_ts,
    {{ landing_parse_audit_epoch('"INGESTED_AT"') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'FED_FDA_DRUG_MASTER_FILES') }} as source
