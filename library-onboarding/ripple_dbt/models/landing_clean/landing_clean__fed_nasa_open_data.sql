{{ config(materialized='view', schema='LANDING_CLEAN', alias='FED_NASA_OPEN_DATA') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.FED_NASA_OPEN_DATA, from the
...
select
    source.*,
    {{ landing_parse_date('"RESPONSE_DATE"', 'iso', 'TEXT') }} as response_date_clean_ts,
    {{ landing_parse_audit_epoch('"_INGESTED_AT"') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'FED_NASA_OPEN_DATA') }} as source
