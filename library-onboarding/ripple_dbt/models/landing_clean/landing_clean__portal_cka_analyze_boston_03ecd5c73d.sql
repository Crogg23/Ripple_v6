{{ config(materialized='view', schema='LANDING_CLEAN', alias='PORTAL_CKA_ANALYZE_BOSTON_03ECD5C73D') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.PORTAL_CKA_ANALYZE_BOSTON_03ECD5C73D, from the
...
select
    source.*,
    {{ landing_parse_date('"DISPATCH_TS"', 'iso', 'TEXT') }} as dispatch_ts_clean_ts,
    {{ landing_parse_date('"INGESTED_AT"', 'iso', 'TEXT') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'PORTAL_CKA_ANALYZE_BOSTON_03ECD5C73D') }} as source
