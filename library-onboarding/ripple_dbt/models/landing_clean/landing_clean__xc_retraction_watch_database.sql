{{ config(materialized='view', schema='LANDING_CLEAN', alias='XC_RETRACTION_WATCH_DATABASE') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.XC_RETRACTION_WATCH_DATABASE, from the
...
select
    source.*,
    {{ landing_parse_date('"RETRACTIONDATE"', 'us', 'TEXT') }} as retractiondate_clean_ts,
    {{ landing_parse_date('"ORIGINALPAPERDATE"', 'us', 'TEXT') }} as originalpaperdate_clean_ts,
    {{ landing_parse_audit_epoch('"INGESTED_AT"') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'XC_RETRACTION_WATCH_DATABASE') }} as source
