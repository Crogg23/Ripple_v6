{{ config(materialized='view', schema='LANDING_CLEAN', alias='FED_USGS_EARTHQUAKES') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.FED_USGS_EARTHQUAKES, from the
...
select
    source.*,
    {{ landing_parse_date('"TIME"', 'iso', 'TEXT') }} as time_clean_ts,
    {{ landing_parse_date('"UPDATED"', 'iso', 'TEXT') }} as updated_clean_ts
from {{ source('ripple_raw', 'FED_USGS_EARTHQUAKES') }} as source
