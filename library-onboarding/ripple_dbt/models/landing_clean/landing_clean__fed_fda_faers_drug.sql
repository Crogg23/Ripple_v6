{{ config(materialized='view', schema='LANDING_CLEAN', alias='FED_FDA_FAERS_DRUG') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.FED_FDA_FAERS_DRUG, from the
...
select
    source.*,
    {{ landing_parse_date('"EXP_DT"', 'ymd8', 'TEXT') }} as exp_dt_clean_ts
from {{ source('ripple_raw', 'FED_FDA_FAERS_DRUG') }} as source
