{{ config(materialized='view', schema='LANDING_CLEAN', alias='ST_OPENSTATES_LEGISLATORS') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.ST_OPENSTATES_LEGISLATORS, from the
...
select
    source.*,
    {{ landing_parse_date('"BIRTH_DATE"', 'iso', 'TEXT') }} as birth_date_clean_ts,
    {{ landing_parse_audit_epoch('"INGESTED_AT"') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'ST_OPENSTATES_LEGISLATORS') }} as source
