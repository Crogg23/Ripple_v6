{{ config(materialized='view', schema='LANDING_CLEAN', alias='FED_ICIJ_OFFSHORELEAKS_RELATIONSHIPS') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.FED_ICIJ_OFFSHORELEAKS_RELATIONSHIPS, from the
...
select
    ...
    {{ landing_parse_date('"END_DATE"', 'dmon', 'TEXT') }} as end_date_clean_ts,
    {{ landing_parse_audit_epoch('"INGESTED_AT"') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'FED_ICIJ_OFFSHORELEAKS_RELATIONSHIPS') }} as source
