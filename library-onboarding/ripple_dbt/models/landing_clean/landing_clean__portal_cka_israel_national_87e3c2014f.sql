{{ config(materialized='view', schema='LANDING_CLEAN', alias='PORTAL_CKA_ISRAEL_NATIONAL_87E3C2014F') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.PORTAL_CKA_ISRAEL_NATIONAL_87E3C2014F, from the
...
select
    source.*,
    {{ landing_parse_date('"OPEN_DATE"', 'us', 'TEXT') }} as open_date_clean_ts,
    {{ landing_parse_date('"CLOSE_DATE"', 'us', 'TEXT') }} as close_date_clean_ts,
    {{ landing_parse_date('"INGESTED_AT"', 'iso', 'TEXT') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'PORTAL_CKA_ISRAEL_NATIONAL_87E3C2014F') }} as source
