{{ config(materialized='view', schema='LANDING_CLEAN', alias='PORTAL_CKA_WESTERN_PENNSYLV_9EF4C60F58') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.PORTAL_CKA_WESTERN_PENNSYLV_9EF4C60F58, from the
...
select
    source.*,
    {{ landing_parse_date('"IN_SERVICE_UTC"', 'iso', 'TEXT') }} as in_service_utc_clean_ts,
    {{ landing_parse_date('"INGESTED_AT"', 'iso', 'TEXT') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'PORTAL_CKA_WESTERN_PENNSYLV_9EF4C60F58') }} as source
