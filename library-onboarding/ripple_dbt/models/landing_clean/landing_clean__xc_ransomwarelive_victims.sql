{{ config(materialized='view', schema='LANDING_CLEAN', alias='XC_RANSOMWARELIVE_VICTIMS') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.XC_RANSOMWARELIVE_VICTIMS, from the
...
select
    source.*,
    {{ landing_parse_date('"DISCOVERED"', 'iso', 'TEXT') }} as discovered_clean_ts,
    {{ landing_parse_date('"PUBLISHED"', 'iso', 'TEXT') }} as published_clean_ts,
    {{ landing_parse_audit_epoch('"INGESTED_AT"') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'XC_RANSOMWARELIVE_VICTIMS') }} as source
