{{ config(materialized='view', schema='LANDING_CLEAN', alias='FED_CMS_HOSPITAL_ENROLLMENTS') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.FED_CMS_HOSPITAL_ENROLLMENTS, from the
...
select
    source.*,
    {{ landing_parse_date('"INCORPORATION_DATE"', 'us', 'TEXT') }} as incorporation_date_clean_ts,
    {{ landing_parse_date('"REH_CONVERSION_DATE"', 'us', 'TEXT') }} as reh_conversion_date_clean_ts
from {{ source('ripple_raw', 'FED_CMS_HOSPITAL_ENROLLMENTS') }} as source
