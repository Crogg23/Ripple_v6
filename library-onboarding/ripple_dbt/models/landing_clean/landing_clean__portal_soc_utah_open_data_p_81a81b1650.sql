{{ config(materialized='view', schema='LANDING_CLEAN', alias='PORTAL_SOC_UTAH_OPEN_DATA_P_81A81B1650') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.PORTAL_SOC_UTAH_OPEN_DATA_P_81A81B1650, from the
...
select
    source.*,
    {{ landing_parse_date('"SOURCEDATE"', 'iso', 'TEXT') }} as sourcedate_clean_ts,
    {{ landing_parse_date('"VAL_DATE"', 'iso', 'TEXT') }} as val_date_clean_ts,
    {{ landing_parse_date('"INGESTED_AT"', 'iso', 'TEXT') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'PORTAL_SOC_UTAH_OPEN_DATA_P_81A81B1650') }} as source
