{{ config(materialized='view', schema='LANDING_CLEAN', alias='FED_HHS_OIG_LEIE') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.FED_HHS_OIG_LEIE, from the
...
select
    source.*,
    {{ landing_parse_date('"DOB"', 'ymd8', 'TEXT') }} as dob_clean_ts,
    {{ landing_parse_date('"EXCLDATE"', 'ymd8', 'TEXT') }} as excldate_clean_ts,
    {{ landing_parse_audit_epoch('"INGESTED_AT"') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'FED_HHS_OIG_LEIE') }} as source
