{{ config(materialized='view', schema='LANDING_CLEAN', alias='PORTAL_CKA_CALIFORNIA_OPEN_D33A75D890') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.PORTAL_CKA_CALIFORNIA_OPEN_D33A75D890, from the
...
select
    source.*,
    {{ landing_parse_date('"EXPIRATION_DATE"', 'us', 'TEXT') }} as expiration_date_clean_ts,
    {{ landing_parse_date('"INGESTED_AT"', 'iso', 'TEXT') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'PORTAL_CKA_CALIFORNIA_OPEN_D33A75D890') }} as source
