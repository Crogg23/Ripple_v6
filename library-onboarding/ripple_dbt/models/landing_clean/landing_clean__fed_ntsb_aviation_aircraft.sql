{{ config(materialized='view', schema='LANDING_CLEAN', alias='FED_NTSB_AVIATION_AIRCRAFT') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.FED_NTSB_AVIATION_AIRCRAFT, from the
...
select
    source.*,
    {{ landing_parse_date('"DATE_LAST_INSP"', 'iso', 'TEXT') }} as date_last_insp_clean_ts,
    {{ landing_parse_date('"LCHG_DATE"', 'iso', 'TEXT') }} as lchg_date_clean_ts,
    {{ landing_parse_audit_epoch('"INGESTED_AT"') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'FED_NTSB_AVIATION_AIRCRAFT') }} as source
