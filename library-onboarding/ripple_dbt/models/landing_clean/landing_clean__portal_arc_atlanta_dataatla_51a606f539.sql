{{ config(materialized='view', schema='LANDING_CLEAN', alias='PORTAL_ARC_ATLANTA_DATAATLA_51A606F539') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.PORTAL_ARC_ATLANTA_DATAATLA_51A606F539, from the
...
select
    source.*,
    {{ landing_parse_date('"DATE_OF_OPENING_IN_ATLANTA"', 'iso', 'TEXT') }} as date_of_opening_in_atlanta_clean_ts,
    {{ landing_parse_date('"INGESTED_AT"', 'iso', 'TEXT') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'PORTAL_ARC_ATLANTA_DATAATLA_51A606F539') }} as source
