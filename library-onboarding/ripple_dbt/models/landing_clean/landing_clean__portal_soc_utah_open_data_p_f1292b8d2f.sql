{{ config(materialized='view', schema='LANDING_CLEAN', alias='PORTAL_SOC_UTAH_OPEN_DATA_P_F1292B8D2F') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.PORTAL_SOC_UTAH_OPEN_DATA_P_F1292B8D2F, from the
...
select
    source.*,
    {{ landing_parse_date('"EVALUATION_CLOSED_DATE"', 'iso', 'TEXT') }} as evaluation_closed_date_clean_ts,
    {{ landing_parse_date('"INGESTED_AT"', 'iso', 'TEXT') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'PORTAL_SOC_UTAH_OPEN_DATA_P_F1292B8D2F') }} as source
