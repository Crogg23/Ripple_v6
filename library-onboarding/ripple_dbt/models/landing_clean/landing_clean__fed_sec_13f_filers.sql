{{ config(materialized='view', schema='LANDING_CLEAN', alias='FED_SEC_13F_FILERS') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.FED_SEC_13F_FILERS, from the
...
select
    source.*,
    {{ landing_parse_date('"REPORTCALENDARORQUARTER"', 'dmon', 'TEXT') }} as reportcalendarorquarter_clean_ts,
    {{ landing_parse_date('"DATEDENIEDEXPIRED"', 'dmon', 'TEXT') }} as datedeniedexpired_clean_ts,
    {{ landing_parse_date('"DATEREPORTED"', 'dmon', 'TEXT') }} as datereported_clean_ts
from {{ source('ripple_raw', 'FED_SEC_13F_FILERS') }} as source
