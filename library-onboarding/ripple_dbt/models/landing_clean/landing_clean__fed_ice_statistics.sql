{{ config(materialized='view', schema='LANDING_CLEAN', alias='FED_ICE_STATISTICS') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.FED_ICE_STATISTICS, from the
...
select
    source.*,
    {{ landing_parse_date('"SNAPSHOT_DATE"', 'iso', 'TEXT') }} as snapshot_date_clean_ts,
    {{ landing_parse_audit_epoch('"_INGESTED_AT"') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'FED_ICE_STATISTICS') }} as source
