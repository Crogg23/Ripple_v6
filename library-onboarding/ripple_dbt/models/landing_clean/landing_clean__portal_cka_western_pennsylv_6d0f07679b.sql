{{ config(materialized='view', schema='LANDING_CLEAN', alias='PORTAL_CKA_WESTERN_PENNSYLV_6D0F07679B') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.PORTAL_CKA_WESTERN_PENNSYLV_6D0F07679B, from the
...
select
    source.*,
    {{ landing_parse_date('"REPORTEDDATE"', 'iso', 'TEXT') }} as reporteddate_clean_ts,
    {{ landing_parse_date('"INGESTED_AT"', 'iso', 'TEXT') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'PORTAL_CKA_WESTERN_PENNSYLV_6D0F07679B') }} as source
