{{ config(materialized='view', schema='LANDING_CLEAN', alias='PORTAL_CKA_CALIFORNIA_OPEN_BFFAFC771D') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.PORTAL_CKA_CALIFORNIA_OPEN_BFFAFC771D, from the
...
select
    source.*,
    {{ landing_parse_date('"DATE_RECEIVED_ACQUIRED"', 'us', 'TEXT') }} as date_received_acquired_clean_ts,
    {{ landing_parse_date('"INGESTED_AT"', 'iso', 'TEXT') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'PORTAL_CKA_CALIFORNIA_OPEN_BFFAFC771D') }} as source
