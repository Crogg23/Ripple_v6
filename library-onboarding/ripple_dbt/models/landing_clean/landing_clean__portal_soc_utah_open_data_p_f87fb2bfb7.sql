{{ config(materialized='view', schema='LANDING_CLEAN', alias='PORTAL_SOC_UTAH_OPEN_DATA_P_F87FB2BFB7') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.PORTAL_SOC_UTAH_OPEN_DATA_P_F87FB2BFB7, from the
...
select
    source.*,
    {{ landing_parse_date('"FINAL_CLOSE_DATE"', 'iso', 'TEXT') }} as final_close_date_clean_ts,
    {{ landing_parse_date('"INGESTED_AT"', 'iso', 'TEXT') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'PORTAL_SOC_UTAH_OPEN_DATA_P_F87FB2BFB7') }} as source
