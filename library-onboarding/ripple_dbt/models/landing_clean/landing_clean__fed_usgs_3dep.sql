{{ config(materialized='view', schema='LANDING_CLEAN', alias='FED_USGS_3DEP') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.FED_USGS_3DEP, from the
...
select
    source.*,
    {{ landing_parse_date('"ACQUISITIONDATE"', 'epochms', 'TEXT') }} as acquisitiondate_clean_ts,
    {{ landing_parse_date('"PUBDATE"', 'ymd8', 'TEXT') }} as pubdate_clean_ts,
    {{ landing_parse_audit_epoch('"_INGESTED_AT"') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'FED_USGS_3DEP') }} as source
