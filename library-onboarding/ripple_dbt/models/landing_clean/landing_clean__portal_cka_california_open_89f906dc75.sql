{{ config(materialized='view', schema='LANDING_CLEAN', alias='PORTAL_CKA_CALIFORNIA_OPEN_89F906DC75') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.PORTAL_CKA_CALIFORNIA_OPEN_89F906DC75, from the
...
select
    source.*,
    {{ landing_parse_date('"DTMBOARDAP"', 'iso', 'TEXT') }} as dtmboardap_clean_ts,
    {{ landing_parse_date('"INGESTED_AT"', 'iso', 'TEXT') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'PORTAL_CKA_CALIFORNIA_OPEN_89F906DC75') }} as source
