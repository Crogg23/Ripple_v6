{{ config(materialized='view', schema='LANDING_CLEAN', alias='FED_FEC_PAC_SUMMARY') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.FED_FEC_PAC_SUMMARY, from the
...
select
    source.*,
    {{ landing_parse_date('"C27"', 'us', 'TEXT') }} as c27_clean_ts
from {{ source('ripple_raw', 'FED_FEC_PAC_SUMMARY') }} as source
