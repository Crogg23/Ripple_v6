{{ config(materialized='view', schema='LANDING_CLEAN', alias='PORTAL_ARC_OPEN_DATA_DC_5B867C795C') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.PORTAL_ARC_OPEN_DATA_DC_5B867C795C, from the
...
select
    source.*,
    {{ landing_parse_date('"DATE"', 'epochms', 'TEXT') }} as date_clean_ts,
    {{ landing_parse_date('"INGESTED_AT"', 'iso', 'TEXT') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'PORTAL_ARC_OPEN_DATA_DC_5B867C795C') }} as source
