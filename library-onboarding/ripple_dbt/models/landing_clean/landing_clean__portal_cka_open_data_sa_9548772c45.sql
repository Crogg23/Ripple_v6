{{ config(materialized='view', schema='LANDING_CLEAN', alias='PORTAL_CKA_OPEN_DATA_SA_9548772C45') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.PORTAL_CKA_OPEN_DATA_SA_9548772C45, from the
...
select
    source.*,
    {{ landing_parse_date('"YEARDESIGNATED"', 'us', 'TEXT') }} as yeardesignated_clean_ts,
    {{ landing_parse_date('"INGESTED_AT"', 'iso', 'TEXT') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'PORTAL_CKA_OPEN_DATA_SA_9548772C45') }} as source
