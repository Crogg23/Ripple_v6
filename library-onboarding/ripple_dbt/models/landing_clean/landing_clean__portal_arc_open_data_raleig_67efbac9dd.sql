{{ config(materialized='view', schema='LANDING_CLEAN', alias='PORTAL_ARC_OPEN_DATA_RALEIG_67EFBAC9DD') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.PORTAL_ARC_OPEN_DATA_RALEIG_67EFBAC9DD, from the
...
select
    source.*,
    {{ landing_parse_date('"APPLICATION_DATE"', 'us', 'TEXT') }} as application_date_clean_ts,
    {{ landing_parse_date('"INGESTED_AT"', 'iso', 'TEXT') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'PORTAL_ARC_OPEN_DATA_RALEIG_67EFBAC9DD') }} as source
