{{ config(materialized='view', schema='LANDING_CLEAN', alias='FED_CFTC_COT_FUTURES') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.FED_CFTC_COT_FUTURES, from the
...
select
    source.*,
    {{ landing_parse_date('"AS_OF_DATE_IN_FORM_YYYY_MM_DD"', 'iso', 'TEXT') }} as as_of_date_in_form_yyyy_mm_dd_clean_ts,
    {{ landing_parse_audit_epoch('"INGESTED_AT"') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'FED_CFTC_COT_FUTURES') }} as source
