{{ config(materialized='view', schema='LANDING_CLEAN', alias='FED_EIA861_MERGERS') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.FED_EIA861_MERGERS, from the
...
select
    source.*,
    {{ landing_parse_date('"C_03_01_2024"', 'us', 'TEXT') }} as c_03_01_2024_clean_ts,
    {{ landing_parse_date('"_INGESTED_AT"', 'iso', 'TEXT') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'FED_EIA861_MERGERS') }} as source
