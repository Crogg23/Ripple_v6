{{ config(materialized='view', schema='LANDING_CLEAN', alias='INTL_IPC_FOOD_INSECURITY_GLOBAL') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.INTL_IPC_FOOD_INSECURITY_GLOBAL, from the
...
select
    source.*,
    {{ landing_parse_date('"C_FROM"', 'iso', 'TEXT') }} as c_from_clean_ts,
    {{ landing_parse_date('"C_TO"', 'iso', 'TEXT') }} as c_to_clean_ts,
    {{ landing_parse_audit_epoch('"INGESTED_AT"') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'INTL_IPC_FOOD_INSECURITY_GLOBAL') }} as source
