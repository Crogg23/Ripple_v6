{{ config(materialized='view', schema='LANDING_CLEAN', alias='PORTAL_SOC_TEXAS_OPEN_DATA_91F1C8D27D') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.PORTAL_SOC_TEXAS_OPEN_DATA_91F1C8D27D, from the
...
select
    source.*,
    {{ landing_parse_date('"ASSOCIATION_BEGIN_DATE"', 'iso', 'TEXT') }} as association_begin_date_clean_ts,
    {{ landing_parse_date('"INGESTED_AT"', 'iso', 'TEXT') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'PORTAL_SOC_TEXAS_OPEN_DATA_91F1C8D27D') }} as source
