{{ config(materialized='view', schema='LANDING_CLEAN', alias='PORTAL_SOC_UTAH_OPEN_DATA_P_D903A6FE0D') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.PORTAL_SOC_UTAH_OPEN_DATA_P_D903A6FE0D, from the
...
select
    source.*,
    {{ landing_parse_date('"CSHO"', 'dmon', 'TEXT') }} as csho_clean_ts,
    {{ landing_parse_date('"DATE_SAMPLED"', 'dmon', 'TEXT') }} as date_sampled_clean_ts,
    {{ landing_parse_date('"INGESTED_AT"', 'iso', 'TEXT') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'PORTAL_SOC_UTAH_OPEN_DATA_P_D903A6FE0D') }} as source
