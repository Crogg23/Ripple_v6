{{ config(materialized='view', schema='LANDING_CLEAN', alias='FED_EPA_RCRA_EVALUATIONS') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.FED_EPA_RCRA_EVALUATIONS, from the
...
select
    source.*,
    {{ landing_parse_date('"EVALUATION_START_DATE"', 'us', 'TEXT') }} as evaluation_start_date_clean_ts,
    {{ landing_parse_audit_epoch('"INGESTED_AT"') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'FED_EPA_RCRA_EVALUATIONS') }} as source
