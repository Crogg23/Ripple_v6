{{ config(materialized='view', schema='LANDING_CLEAN', alias='ST_NYC_CFB_CAMPAIGN_2021_CONTRIBUTIONS') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.ST_NYC_CFB_CAMPAIGN_2021_CONTRIBUTIONS, from the
...
select
    ...
    {{ landing_parse_date('"REFUNDDATE"', 'us', 'TEXT') }} as refunddate_clean_ts,
    {{ landing_parse_audit_epoch('"INGESTED_AT"') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'ST_NYC_CFB_CAMPAIGN_2021_CONTRIBUTIONS') }} as source
