{{ config(materialized='view', schema='LANDING_CLEAN', alias='PORTAL_CKA_ANALYZE_BOSTON_4A5CA3BCE5') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.PORTAL_CKA_ANALYZE_BOSTON_4A5CA3BCE5, from the
...
select
    source.*,
    {{ landing_parse_date('"ETL_UPDATEDTIMESTAMP"', 'us', 'TEXT') }} as etl_updatedtimestamp_clean_ts,
    {{ landing_parse_date('"INGESTED_AT"', 'iso', 'TEXT') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'PORTAL_CKA_ANALYZE_BOSTON_4A5CA3BCE5') }} as source
