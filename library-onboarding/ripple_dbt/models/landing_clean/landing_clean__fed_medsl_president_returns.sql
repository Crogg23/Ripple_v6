{{ config(materialized='view', schema='LANDING_CLEAN', alias='FED_MEDSL_PRESIDENT_RETURNS') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.FED_MEDSL_PRESIDENT_RETURNS, from the
...
select
    source.*,
    {{ landing_parse_date('"VERSION"', 'ymd8', 'TEXT') }} as version_clean_ts,
    {{ landing_parse_audit_epoch('"_INGESTED_AT"') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'FED_MEDSL_PRESIDENT_RETURNS') }} as source
