{{ config(materialized='view', schema='LANDING_CLEAN', alias='FED_CMS_HCRIS') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.FED_CMS_HCRIS, from the
...
select
    source.*,
    {{ landing_parse_date('"FISCAL_YEAR_BEGIN_DATE"', 'us', 'TEXT') }} as fiscal_year_begin_date_clean_ts,
    {{ landing_parse_date('"FISCAL_YEAR_END_DATE"', 'us', 'TEXT') }} as fiscal_year_end_date_clean_ts,
    {{ landing_parse_audit_epoch('"_INGESTED_AT"') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'FED_CMS_HCRIS') }} as source
