{{ config(materialized='view', schema='LANDING_CLEAN', alias='PORTAL_CKA_WESTERN_PENNSYLV_2DFC1ADDEA') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.PORTAL_CKA_WESTERN_PENNSYLV_2DFC1ADDEA, from the
...
select
    source.*,
    {{ landing_parse_date('"DEATH_DATE_AND_TIME"', 'iso', 'TEXT') }} as death_date_and_time_clean_ts,
    {{ landing_parse_date('"INGESTED_AT"', 'iso', 'TEXT') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'PORTAL_CKA_WESTERN_PENNSYLV_2DFC1ADDEA') }} as source
