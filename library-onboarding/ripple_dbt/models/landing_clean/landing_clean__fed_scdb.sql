{{ config(materialized='view', schema='LANDING_CLEAN', alias='FED_SCDB') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.FED_SCDB, from the
...
select
    source.*,
    {{ landing_parse_date('"DATEDECISION"', 'us', 'TEXT') }} as datedecision_clean_ts,
    {{ landing_parse_date('"DATEARGUMENT"', 'us', 'TEXT') }} as dateargument_clean_ts,
    {{ landing_parse_date('"DATEREARG"', 'us', 'TEXT') }} as daterearg_clean_ts,
    {{ landing_parse_audit_epoch('"_INGESTED_AT"') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'FED_SCDB') }} as source
