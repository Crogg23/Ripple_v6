{{ config(materialized='view', schema='LANDING_CLEAN', alias='FED_OYEZ') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.FED_OYEZ, from the
...
select
    source.*,
    {{ landing_parse_date('"ARGUMENT_DATE"', 'iso', 'TEXT') }} as argument_date_clean_ts,
    {{ landing_parse_date('"DECISION_DATE"', 'iso', 'TEXT') }} as decision_date_clean_ts,
    {{ landing_parse_date('"SCRAPED_AT"', 'iso', 'TEXT') }} as scraped_at_clean_ts,
    {{ landing_parse_audit_epoch('"_INGESTED_AT"') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'FED_OYEZ') }} as source
