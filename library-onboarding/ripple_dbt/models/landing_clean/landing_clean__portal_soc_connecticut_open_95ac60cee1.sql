{{ config(materialized='view', schema='LANDING_CLEAN', alias='PORTAL_SOC_CONNECTICUT_OPEN_95AC60CEE1') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.PORTAL_SOC_CONNECTICUT_OPEN_95AC60CEE1, from the
...
select
    source.*,
    {{ landing_parse_date('"PATENTDATE"', 'iso', 'TEXT') }} as patentdate_clean_ts,
    {{ landing_parse_date('"INGESTED_AT"', 'iso', 'TEXT') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'PORTAL_SOC_CONNECTICUT_OPEN_95AC60CEE1') }} as source
