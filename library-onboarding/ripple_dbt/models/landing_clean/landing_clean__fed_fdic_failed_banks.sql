{{ config(materialized='view', schema='LANDING_CLEAN', alias='FED_FDIC_FAILED_BANKS') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.FED_FDIC_FAILED_BANKS, from the
...
select
    source.*,
    {{ landing_parse_date('"RESDATE"', 'us', 'TEXT') }} as resdate_clean_ts,
    {{ landing_parse_date('"FAILDATE"', 'us', 'TEXT') }} as faildate_clean_ts,
    {{ landing_parse_audit_epoch('"_INGESTED_AT"') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'FED_FDIC_FAILED_BANKS') }} as source
