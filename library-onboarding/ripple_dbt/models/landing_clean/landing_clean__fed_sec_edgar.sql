{{ config(materialized='view', schema='LANDING_CLEAN', alias='FED_SEC_EDGAR') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.FED_SEC_EDGAR, from the
...
select
    source.*,
    {{ landing_parse_date('"FILEDAT"', 'iso', 'TEXT') }} as filedat_clean_ts,
...
from {{ source('ripple_raw', 'FED_SEC_EDGAR') }} as source
