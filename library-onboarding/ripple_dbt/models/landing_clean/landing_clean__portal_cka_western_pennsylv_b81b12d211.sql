{{ config(materialized='view', schema='LANDING_CLEAN', alias='PORTAL_CKA_WESTERN_PENNSYLV_B81B12D211') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.PORTAL_CKA_WESTERN_PENNSYLV_B81B12D211, from the
...
select
    source.*,
    {{ landing_parse_date('"ALARM_TIME"', 'iso', 'TEXT') }} as alarm_time_clean_ts,
    {{ landing_parse_date('"INGESTED_AT"', 'iso', 'TEXT') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'PORTAL_CKA_WESTERN_PENNSYLV_B81B12D211') }} as source
