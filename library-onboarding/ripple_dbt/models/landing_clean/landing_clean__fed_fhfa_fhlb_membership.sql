{{ config(materialized='view', schema='LANDING_CLEAN', alias='FED_FHFA_FHLB_MEMBERSHIP') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.FED_FHFA_FHLB_MEMBERSHIP, from the
...
select
    ...
    {{ landing_parse_date('"MEM_DATE"', 'us', 'TEXT') }} as mem_date_clean_ts,
    {{ landing_parse_audit_epoch('"INGESTED_AT"') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'FED_FHFA_FHLB_MEMBERSHIP') }} as source
