{{ config(materialized='view', schema='LANDING_CLEAN', alias='PORTAL_SOC_CONNECTICUT_OPEN_9A34CD28AE') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.PORTAL_SOC_CONNECTICUT_OPEN_9A34CD28AE, from the
...
select
    source.*,
    {{ landing_parse_date('"COLLECTION_WEEK"', 'iso', 'TEXT') }} as collection_week_clean_ts,
    {{ landing_parse_date('"INGESTED_AT"', 'iso', 'TEXT') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'PORTAL_SOC_CONNECTICUT_OPEN_9A34CD28AE') }} as source
