{{ config(materialized='view', schema='LANDING_CLEAN', alias='FED_HOUSE_FD_PTR_INDEX') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.FED_HOUSE_FD_PTR_INDEX, from the
...
select
    source.*,
    {{ landing_parse_date('"FILINGDATE"', 'us', 'TEXT') }} as filingdate_clean_ts,
    {{ landing_parse_audit_epoch('"INGESTED_AT"') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'FED_HOUSE_FD_PTR_INDEX') }} as source
