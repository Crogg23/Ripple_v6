{{ config(materialized='view', schema='LANDING_CLEAN', alias='FED_HUD_MF_FIRM_COMMITMENTS') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.FED_HUD_MF_FIRM_COMMITMENTS, from the
...
select
    source.*,
    {{ landing_parse_date('"UNNAMED_14"', 'iso', 'TEXT') }} as unnamed_14_clean_ts,
    {{ landing_parse_audit_epoch('"INGESTED_AT"') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'FED_HUD_MF_FIRM_COMMITMENTS') }} as source
