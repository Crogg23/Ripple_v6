{{ config(materialized='view', schema='LANDING_CLEAN', alias='FED_EPA_CAMPD_FACILITY') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.FED_EPA_CAMPD_FACILITY, from the
...
select
    source.*,
    {{ landing_parse_date('"COMMERCIAL_OPERATION_DATE"', 'iso', 'TEXT') }} as commercial_operation_date_clean_ts
from {{ source('ripple_raw', 'FED_EPA_CAMPD_FACILITY') }} as source
