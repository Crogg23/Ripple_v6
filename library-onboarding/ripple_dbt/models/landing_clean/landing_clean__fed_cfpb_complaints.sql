{{ config(materialized='view', schema='LANDING_CLEAN', alias='FED_CFPB_COMPLAINTS') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.FED_CFPB_COMPLAINTS, from the
...
select
    source.*,
    {{ landing_parse_date('"Date received"', 'iso', 'TEXT') }} as date_received_clean_ts,
    {{ landing_parse_date('"Date sent to company"', 'iso', 'TEXT') }} as date_sent_to_company_clean_ts
from {{ source('ripple_raw', 'FED_CFPB_COMPLAINTS') }} as source
