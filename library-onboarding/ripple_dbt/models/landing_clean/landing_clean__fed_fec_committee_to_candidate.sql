{{ config(materialized='view', schema='LANDING_CLEAN', alias='FED_FEC_COMMITTEE_TO_CANDIDATE') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.FED_FEC_COMMITTEE_TO_CANDIDATE, from the
...
select
    source.*,
    {{ landing_parse_date('"TRANSACTION_DT"', 'mdy8', 'TEXT') }} as transaction_dt_clean_ts,
    {{ landing_parse_audit_epoch('"_INGESTED_AT"') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'FED_FEC_COMMITTEE_TO_CANDIDATE') }} as source
