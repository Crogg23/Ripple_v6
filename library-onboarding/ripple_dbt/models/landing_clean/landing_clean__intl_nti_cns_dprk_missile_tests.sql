{{ config(materialized='view', schema='LANDING_CLEAN', alias='INTL_NTI_CNS_DPRK_MISSILE_TESTS') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.INTL_NTI_CNS_DPRK_MISSILE_TESTS, from the
...
select
    ...
    {{ landing_parse_date('"DATE_ENTERED_UPDATED"', 'iso', 'TEXT') }} as date_entered_updated_clean_ts,
    {{ landing_parse_audit_epoch('"INGESTED_AT"') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'INTL_NTI_CNS_DPRK_MISSILE_TESTS') }} as source
