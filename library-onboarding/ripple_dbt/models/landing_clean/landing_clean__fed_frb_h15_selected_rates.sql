{{ config(materialized='view', schema='LANDING_CLEAN', alias='FED_FRB_H15_SELECTED_RATES') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.FED_FRB_H15_SELECTED_RATES, from the
...
select
    source.*,
    {{ landing_parse_date('"SERIES_DESCRIPTION"', 'iso', 'TEXT') }} as series_description_clean_ts,
    {{ landing_parse_audit_epoch('"INGESTED_AT"') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'FED_FRB_H15_SELECTED_RATES') }} as source
