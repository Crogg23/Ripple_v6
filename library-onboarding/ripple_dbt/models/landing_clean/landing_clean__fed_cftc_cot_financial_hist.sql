{{ config(materialized='view', schema='LANDING_CLEAN', alias='FED_CFTC_COT_FINANCIAL_HIST') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.FED_CFTC_COT_FINANCIAL_HIST, from the
...
select
    source.*,
    {{ landing_parse_date('"REPORT_DATE_AS_MM_DD_YYYY"', 'iso', 'TEXT') }} as report_date_as_mm_dd_yyyy_clean_ts,
    {{ landing_parse_audit_epoch('"INGESTED_AT"') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'FED_CFTC_COT_FINANCIAL_HIST') }} as source
