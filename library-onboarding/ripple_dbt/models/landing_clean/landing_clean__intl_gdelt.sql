{{ config(materialized='view', schema='LANDING_CLEAN', alias='INTL_GDELT') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.INTL_GDELT, from the
...
select
    source.*,
    {{ landing_parse_date('"SQLDATE"', 'ymd8', 'TEXT') }} as sqldate_clean_ts,
    {{ landing_parse_audit_epoch('"_INGESTED_AT"') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'INTL_GDELT') }} as source
