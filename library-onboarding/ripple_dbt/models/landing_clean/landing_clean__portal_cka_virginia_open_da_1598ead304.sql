{{ config(materialized='view', schema='LANDING_CLEAN', alias='PORTAL_CKA_VIRGINIA_OPEN_DA_1598EAD304') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.PORTAL_CKA_VIRGINIA_OPEN_DA_1598EAD304, from the
...
select
    source.*,
    {{ landing_parse_date('"WEEK_ENDING_DATE"', 'iso', 'TEXT') }} as week_ending_date_clean_ts,
    {{ landing_parse_date('"INGESTED_AT"', 'iso', 'TEXT') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'PORTAL_CKA_VIRGINIA_OPEN_DA_1598EAD304') }} as source
