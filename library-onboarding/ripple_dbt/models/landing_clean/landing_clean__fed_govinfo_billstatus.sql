{{ config(materialized='view', schema='LANDING_CLEAN', alias='FED_GOVINFO_BILLSTATUS') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.FED_GOVINFO_BILLSTATUS, from the
...
select
    source.*,
    {{ landing_parse_date('"INTRODUCED_DATE"', 'iso', 'TEXT') }} as introduced_date_clean_ts,
    {{ landing_parse_date('"LATEST_ACTION_DATE"', 'iso', 'TEXT') }} as latest_action_date_clean_ts,
    {{ landing_parse_audit_epoch('"_INGESTED_AT"') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'FED_GOVINFO_BILLSTATUS') }} as source
