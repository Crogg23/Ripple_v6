{{ config(materialized='view', schema='LANDING_CLEAN', alias='FED_ITIS_COMMENTS') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.FED_ITIS_COMMENTS, from the
...
select
    source.*,
    {{ landing_parse_date('"COMMENT_TIME_STAMP"', 'iso', 'TEXT') }} as comment_time_stamp_clean_ts,
...
from {{ source('ripple_raw', 'FED_ITIS_COMMENTS') }} as source
