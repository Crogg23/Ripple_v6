{{ config(materialized='view', schema='LANDING_CLEAN', alias='PORTAL_ARC_NEW_MEXICO_OPEN_7DE8F482F1') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.PORTAL_ARC_NEW_MEXICO_OPEN_7DE8F482F1, from the
...
select
    source.*,
    {{ landing_parse_date('"DATA_COLLE"', 'epochms', 'TEXT') }} as data_colle_clean_ts,
    {{ landing_parse_date('"INGESTED_AT"', 'iso', 'TEXT') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'PORTAL_ARC_NEW_MEXICO_OPEN_7DE8F482F1') }} as source
