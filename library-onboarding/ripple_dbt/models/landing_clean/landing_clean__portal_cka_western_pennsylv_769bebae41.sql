{{ config(materialized='view', schema='LANDING_CLEAN', alias='PORTAL_CKA_WESTERN_PENNSYLV_769BEBAE41') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.PORTAL_CKA_WESTERN_PENNSYLV_769BEBAE41, from the
...
select
    source.*,
    {{ landing_parse_date('"STARTTIME"', 'us', 'TEXT') }} as starttime_clean_ts,
    {{ landing_parse_date('"STOPTIME"', 'us', 'TEXT') }} as stoptime_clean_ts,
    {{ landing_parse_date('"INGESTED_AT"', 'iso', 'TEXT') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'PORTAL_CKA_WESTERN_PENNSYLV_769BEBAE41') }} as source
