{{ config(materialized='view', schema='LANDING_CLEAN', alias='STATE_MO_SEX_OFFENDER_REGISTRY') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.STATE_MO_SEX_OFFENDER_REGISTRY, from the
...
select
    source.*,
    {{ landing_parse_date('"DATE_OF_BIRTH"', 'iso', 'TEXT') }} as date_of_birth_clean_ts,
    {{ landing_parse_audit_epoch('"INGESTED_AT"') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'STATE_MO_SEX_OFFENDER_REGISTRY') }} as source
