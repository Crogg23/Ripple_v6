{{ config(materialized='view', schema='LANDING_CLEAN', alias='INTL_IT_ISTAT') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.INTL_IT_ISTAT, from the
...
select
    source.*,
    {{ landing_parse_date('"FETCHED_AT"', 'iso', 'TEXT') }} as fetched_at_clean_ts,
    {{ landing_parse_audit_epoch('"_INGESTED_AT"') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'INTL_IT_ISTAT') }} as source
