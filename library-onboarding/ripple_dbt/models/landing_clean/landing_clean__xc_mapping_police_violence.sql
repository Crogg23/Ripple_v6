{{ config(materialized='view', schema='LANDING_CLEAN', alias='XC_MAPPING_POLICE_VIOLENCE') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.XC_MAPPING_POLICE_VIOLENCE, from the
...
select
    source.*,
    {{ landing_parse_date('"DATE_OF_INCIDENT_MONTH_DAY_YEAR"', 'iso', 'TEXT') }} as date_of_incident_month_day_year_clean_ts,
    {{ landing_parse_audit_epoch('"INGESTED_AT"') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'XC_MAPPING_POLICE_VIOLENCE') }} as source
