{{ config(materialized='view', schema='LANDING_CLEAN', alias='PORTAL_SOC_TEXAS_OPEN_DATA_A7AB49CB1A') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.PORTAL_SOC_TEXAS_OPEN_DATA_A7AB49CB1A, from the
...
select
    source.*,
    {{ landing_parse_date('"DATE_SUBMITTED"', 'iso', 'TEXT') }} as date_submitted_clean_ts,
    {{ landing_parse_date('"INGESTED_AT"', 'iso', 'TEXT') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'PORTAL_SOC_TEXAS_OPEN_DATA_A7AB49CB1A') }} as source
