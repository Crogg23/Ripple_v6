{{ config(materialized='view', schema='LANDING_CLEAN', alias='FED_SEC_MONEY_MARKET_FUND_INFORMATION') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.FED_SEC_MONEY_MARKET_FUND_INFORMATION, from the
...
select
    source.*,
    {{ landing_parse_date('"REPORTMONTH"', 'iso', 'TEXT') }} as reportmonth_clean_ts,
    {{ landing_parse_audit_epoch('"_INGESTED_AT"') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'FED_SEC_MONEY_MARKET_FUND_INFORMATION') }} as source
