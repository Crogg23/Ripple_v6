{{ config(materialized='view', schema='LANDING_CLEAN', alias='STATE_OEHHA_PROP65_CHEMICALS') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.STATE_OEHHA_PROP65_CHEMICALS, from the
...
select
    source.*,
    {{ landing_parse_date('"DATE_LISTED"', 'dmon', 'TEXT') }} as date_listed_clean_ts,
    {{ landing_parse_audit_epoch('"INGESTED_AT"') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'STATE_OEHHA_PROP65_CHEMICALS') }} as source
