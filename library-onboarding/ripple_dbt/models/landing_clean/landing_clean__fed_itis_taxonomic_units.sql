{{ config(materialized='view', schema='LANDING_CLEAN', alias='FED_ITIS_TAXONOMIC_UNITS') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.FED_ITIS_TAXONOMIC_UNITS, from the
...
select
    source.*,
    {{ landing_parse_date('"INITIAL_TIME_STAMP"', 'iso', 'TEXT') }} as initial_time_stamp_clean_ts,
...
from {{ source('ripple_raw', 'FED_ITIS_TAXONOMIC_UNITS') }} as source
