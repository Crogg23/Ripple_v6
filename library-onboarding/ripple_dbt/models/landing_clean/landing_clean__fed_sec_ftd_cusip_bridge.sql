{{ config(materialized='view', schema='LANDING_CLEAN', alias='FED_SEC_FTD_CUSIP_BRIDGE') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.FED_SEC_FTD_CUSIP_BRIDGE, from the
...
select
    source.*,
    {{ landing_parse_date('"SETTLEMENT_DATE"', 'ymd8', 'TEXT') }} as settlement_date_clean_ts,
    {{ landing_parse_audit_epoch('"INGESTED_AT"') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'FED_SEC_FTD_CUSIP_BRIDGE') }} as source
