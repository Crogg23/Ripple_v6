{{ config(materialized='view', schema='LANDING_CLEAN', alias='FED_FEC_API') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.FED_FEC_API, from the
...
select
    source.*,
    {{ landing_parse_date('"CONTRIBUTION_RECEIPT_DATE"', 'iso', 'TEXT') }} as contribution_receipt_date_clean_ts,
    {{ landing_parse_date('"LOAD_DATE"', 'iso', 'TEXT') }} as load_date_clean_ts,
    {{ landing_parse_audit_epoch('"_INGESTED_AT"') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'FED_FEC_API') }} as source
