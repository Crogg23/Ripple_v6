{{ config(materialized='view', schema='LANDING_CLEAN', alias='PORTAL_CKA_WESTERN_PENNSYLV_A3B7349811') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.PORTAL_CKA_WESTERN_PENNSYLV_A3B7349811, from the
...
select
    source.*,
    {{ landing_parse_date('"FROM_DATE"', 'iso', 'TEXT') }} as from_date_clean_ts,
    {{ landing_parse_date('"TO_DATE"', 'iso', 'TEXT') }} as to_date_clean_ts,
    {{ landing_parse_date('"INGESTED_AT"', 'iso', 'TEXT') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'PORTAL_CKA_WESTERN_PENNSYLV_A3B7349811') }} as source
