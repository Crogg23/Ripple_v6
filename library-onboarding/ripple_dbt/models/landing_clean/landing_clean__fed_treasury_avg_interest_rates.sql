{{ config(materialized='view', schema='LANDING_CLEAN', alias='FED_TREASURY_AVG_INTEREST_RATES') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.FED_TREASURY_AVG_INTEREST_RATES, from the
...
select
    source.*,
    {{ landing_parse_date('"RECORD_DATE"', 'iso', 'TEXT') }} as record_date_clean_ts,
    {{ landing_parse_audit_epoch('"_INGESTED_AT"') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'FED_TREASURY_AVG_INTEREST_RATES') }} as source
