{{ config(materialized='view', schema='LANDING_CLEAN', alias='FED_PBGC_TRUSTEED_PENSION_PLANS') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.FED_PBGC_TRUSTEED_PENSION_PLANS, from the
...
select
    source.*,
    {{ landing_parse_date('"PLAN_EFFECTIVE_DATE"', 'us', 'TEXT') }} as plan_effective_date_clean_ts,
    {{ landing_parse_audit_epoch('"INGESTED_AT"') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'FED_PBGC_TRUSTEED_PENSION_PLANS') }} as source
