{{ config(materialized='view', schema='LANDING_CLEAN', alias='FED_EPA_RCRA_ENFORCEMENTS') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.FED_EPA_RCRA_ENFORCEMENTS, from the
...
select
    source.*,
    {{ landing_parse_date('"ENFORCEMENT_ACTION_DATE"', 'us', 'TEXT') }} as enforcement_action_date_clean_ts,
    {{ landing_parse_audit_epoch('"INGESTED_AT"') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'FED_EPA_RCRA_ENFORCEMENTS') }} as source
