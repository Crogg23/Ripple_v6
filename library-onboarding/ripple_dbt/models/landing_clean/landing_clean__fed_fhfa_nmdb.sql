{{ config(materialized='view', schema='LANDING_CLEAN', alias='FED_FHFA_NMDB') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.FED_FHFA_NMDB, from the
...
select
    source.*,
    {{ landing_parse_date('"RELEASE_DATE"', 'iso', 'TEXT') }} as release_date_clean_ts,
    {{ landing_parse_audit_epoch('"_INGESTED_AT"') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'FED_FHFA_NMDB') }} as source
