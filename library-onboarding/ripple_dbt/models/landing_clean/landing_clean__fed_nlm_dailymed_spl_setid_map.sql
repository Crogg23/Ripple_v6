{{ config(materialized='view', schema='LANDING_CLEAN', alias='FED_NLM_DAILYMED_SPL_SETID_MAP') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.FED_NLM_DAILYMED_SPL_SETID_MAP, from the
...
select
    source.*,
    {{ landing_parse_date('"UPLOAD_DATE"', 'us', 'TEXT') }} as upload_date_clean_ts,
    {{ landing_parse_audit_epoch('"INGESTED_AT"') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'FED_NLM_DAILYMED_SPL_SETID_MAP') }} as source
