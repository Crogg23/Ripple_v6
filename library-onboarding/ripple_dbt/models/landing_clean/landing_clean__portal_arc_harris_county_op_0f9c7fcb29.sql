{{ config(materialized='view', schema='LANDING_CLEAN', alias='PORTAL_ARC_HARRIS_COUNTY_OP_0F9C7FCB29') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.PORTAL_ARC_HARRIS_COUNTY_OP_0F9C7FCB29, from the
...
select
    source.*,
    {{ landing_parse_date('"OBSOLDATE"', 'epochs', 'TEXT') }} as obsoldate_clean_ts,
    {{ landing_parse_date('"PRODDATE"', 'ymd8', 'TEXT') }} as proddate_clean_ts,
    {{ landing_parse_date('"INGESTED_AT"', 'iso', 'TEXT') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'PORTAL_ARC_HARRIS_COUNTY_OP_0F9C7FCB29') }} as source
