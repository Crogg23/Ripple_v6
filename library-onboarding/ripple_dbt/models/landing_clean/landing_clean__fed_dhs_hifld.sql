{{ config(materialized='view', schema='LANDING_CLEAN', alias='FED_DHS_HIFLD') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.FED_DHS_HIFLD, from the
...
select
    source.*,
    {{ landing_parse_date('"SOURCE_DATE"', 'epochms', 'TEXT') }} as source_date_clean_ts,
    {{ landing_parse_audit_epoch('"_INGESTED_AT"') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'FED_DHS_HIFLD') }} as source
