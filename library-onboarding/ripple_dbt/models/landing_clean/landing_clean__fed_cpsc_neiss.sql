{{ config(materialized='view', schema='LANDING_CLEAN', alias='FED_CPSC_NEISS') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.FED_CPSC_NEISS, from the
...
select
    source.*,
    {{ landing_parse_date('"TREATMENT_DATE"', 'us', 'TEXT') }} as treatment_date_clean_ts
from {{ source('ripple_raw', 'FED_CPSC_NEISS') }} as source
