{{ config(materialized='view', schema='LANDING_CLEAN', alias='FED_EPA_NPDES_NPDES_FORMAL_ENFORCEMENT_ACTIONS') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.FED_EPA_NPDES_NPDES_FORMAL_ENFORCEMENT_ACTIONS, from the
...
select
    source.*,
    {{ landing_parse_date('"SETTLEMENT_ENTERED_DATE"', 'us', 'TEXT') }} as settlement_entered_date_clean_ts,
    {{ landing_parse_audit_epoch('"_INGESTED_AT"') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'FED_EPA_NPDES_NPDES_FORMAL_ENFORCEMENT_ACTIONS') }} as source
