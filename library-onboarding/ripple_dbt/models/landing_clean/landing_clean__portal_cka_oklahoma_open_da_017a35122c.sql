{{ config(materialized='view', schema='LANDING_CLEAN', alias='PORTAL_CKA_OKLAHOMA_OPEN_DA_017A35122C') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.PORTAL_CKA_OKLAHOMA_OPEN_DA_017A35122C, from the
...
select
    source.*,
    {{ landing_parse_date('"DATE_ACQUIRED_OR_INSTALLED"', 'us', 'TEXT') }} as date_acquired_or_installed_clean_ts,
    {{ landing_parse_date('"INGESTED_AT"', 'iso', 'TEXT') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'PORTAL_CKA_OKLAHOMA_OPEN_DA_017A35122C') }} as source
