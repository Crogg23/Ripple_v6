{{ config(materialized='view', schema='LANDING_CLEAN', alias='PORTAL_CKA_ANALYZE_BOSTON_5DD692715C') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.PORTAL_CKA_ANALYZE_BOSTON_5DD692715C, from the
...
select
    source.*,
    {{ landing_parse_date('"LICENSE_ADD_DT_TM"', 'iso', 'TEXT') }} as license_add_dt_tm_clean_ts,
    {{ landing_parse_date('"INGESTED_AT"', 'iso', 'TEXT') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'PORTAL_CKA_ANALYZE_BOSTON_5DD692715C') }} as source
