{{ config(materialized='view', schema='LANDING_CLEAN', alias='FED_CMS_OPIOID_TREATMENT_PROGRAM_PROVIDERS') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.FED_CMS_OPIOID_TREATMENT_PROGRAM_PROVIDERS, from the
...
select
    source.*,
    {{ landing_parse_date('"MEDICARE_ID_EFFECTIVE_DATE"', 'us', 'TEXT') }} as medicare_id_effective_date_clean_ts
from {{ source('ripple_raw', 'FED_CMS_OPIOID_TREATMENT_PROGRAM_PROVIDERS') }} as source
