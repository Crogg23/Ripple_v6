{{ config(materialized='view', schema='LANDING_CLEAN', alias='PORTAL_CKA_WESTERN_PENNSYLV_D654695651') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.PORTAL_CKA_WESTERN_PENNSYLV_D654695651, from the
...
select
    source.*,
    {{ landing_parse_date('"REQUEST_DATE"', 'iso', 'TEXT') }} as request_date_clean_ts,
    {{ landing_parse_date('"INGESTED_AT"', 'iso', 'TEXT') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'PORTAL_CKA_WESTERN_PENNSYLV_D654695651') }} as source
