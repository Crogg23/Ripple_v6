{{ config(materialized='view', schema='LANDING_CLEAN', alias='FED_NHTSA_INVESTIGATIONS') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.FED_NHTSA_INVESTIGATIONS, from the
...
select
    source.*,
    {{ landing_parse_date('"C7"', 'ymd8', 'TEXT') }} as c7_clean_ts,
    {{ landing_parse_date('"C8"', 'ymd8', 'TEXT') }} as c8_clean_ts
from {{ source('ripple_raw', 'FED_NHTSA_INVESTIGATIONS') }} as source
