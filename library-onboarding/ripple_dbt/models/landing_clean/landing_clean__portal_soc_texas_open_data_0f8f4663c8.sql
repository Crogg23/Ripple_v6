{{ config(materialized='view', schema='LANDING_CLEAN', alias='PORTAL_SOC_TEXAS_OPEN_DATA_0F8F4663C8') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.PORTAL_SOC_TEXAS_OPEN_DATA_0F8F4663C8, from the
...
select
    source.*,
    {{ landing_parse_date('"RESPONSIBILITY_BEGIN_DATE"', 'us', 'TEXT') }} as responsibility_begin_date_clean_ts,
    {{ landing_parse_date('"INGESTED_AT"', 'iso', 'TEXT') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'PORTAL_SOC_TEXAS_OPEN_DATA_0F8F4663C8') }} as source
