{{ config(materialized='view', schema='LANDING_CLEAN', alias='FED_FDA_GUDID_FULL_IDENTIFIERS') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.FED_FDA_GUDID_FULL_IDENTIFIERS, from the
...
select
    source.*,
    {{ landing_parse_date('"PKGDISCONTINUEDATE"', 'iso', 'TEXT') }} as pkgdiscontinuedate_clean_ts
from {{ source('ripple_raw', 'FED_FDA_GUDID_FULL_IDENTIFIERS') }} as source
