{{ config(materialized='view', schema='LANDING_CLEAN', alias='FED_FEC_BULK_SUMMARY') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.FED_FEC_BULK_SUMMARY, from the
...
select
    source.*,
    {{ landing_parse_date('"CVG_END_DT"', 'us', 'TEXT') }} as cvg_end_dt_clean_ts,
    {{ landing_parse_audit_epoch('"_INGESTED_AT"') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'FED_FEC_BULK_SUMMARY') }} as source
