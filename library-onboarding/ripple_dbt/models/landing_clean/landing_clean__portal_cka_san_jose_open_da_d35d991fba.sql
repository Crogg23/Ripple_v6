{{ config(materialized='view', schema='LANDING_CLEAN', alias='PORTAL_CKA_SAN_JOSE_OPEN_DA_D35D991FBA') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.PORTAL_CKA_SAN_JOSE_OPEN_DA_D35D991FBA, from the
...
select
    source.*,
    {{ landing_parse_date('"ISSUEDATE"', 'us', 'TEXT') }} as issuedate_clean_ts,
    {{ landing_parse_date('"FINALDATE"', 'us', 'TEXT') }} as finaldate_clean_ts,
    {{ landing_parse_date('"INGESTED_AT"', 'iso', 'TEXT') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'PORTAL_CKA_SAN_JOSE_OPEN_DA_D35D991FBA') }} as source
