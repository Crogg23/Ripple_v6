{{ config(materialized='view', schema='LANDING_CLEAN', alias='PORTAL_CKA_ISRAEL_NATIONAL_B0A05D8626') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.PORTAL_CKA_ISRAEL_NATIONAL_B0A05D8626, from the
...
select
    source.*,
    {{ landing_parse_date('"APPOINTMENT_DATE"', 'iso', 'TEXT') }} as appointment_date_clean_ts,
    {{ landing_parse_date('"INGESTED_AT"', 'iso', 'TEXT') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'PORTAL_CKA_ISRAEL_NATIONAL_B0A05D8626') }} as source
