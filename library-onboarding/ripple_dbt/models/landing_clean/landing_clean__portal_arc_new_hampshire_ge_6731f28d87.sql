{{ config(materialized='view', schema='LANDING_CLEAN', alias='PORTAL_ARC_NEW_HAMPSHIRE_GE_6731F28D87') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.PORTAL_ARC_NEW_HAMPSHIRE_GE_6731F28D87, from the
...
select
    source.*,
    {{ landing_parse_date('"STAMPDATE"', 'us', 'TEXT') }} as stampdate_clean_ts,
    {{ landing_parse_date('"UPDATED"', 'epochms', 'TEXT') }} as updated_clean_ts,
    {{ landing_parse_date('"INGESTED_AT"', 'iso', 'TEXT') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'PORTAL_ARC_NEW_HAMPSHIRE_GE_6731F28D87') }} as source
