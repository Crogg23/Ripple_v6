{{ config(materialized='view', schema='LANDING_CLEAN', alias='PORTAL_CKA_ANALYZE_BOSTON_5CCB249B71') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.PORTAL_CKA_ANALYZE_BOSTON_5CCB249B71, from the
...
select
    source.*,
    {{ landing_parse_date('"NEW_INSP_D"', 'us', 'TEXT') }} as new_insp_d_clean_ts,
    {{ landing_parse_date('"INSP_DATE"', 'us', 'TEXT') }} as insp_date_clean_ts,
    {{ landing_parse_date('"INGESTED_AT"', 'iso', 'TEXT') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'PORTAL_CKA_ANALYZE_BOSTON_5CCB249B71') }} as source
