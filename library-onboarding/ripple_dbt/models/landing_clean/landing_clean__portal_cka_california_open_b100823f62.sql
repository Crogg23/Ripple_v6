{{ config(materialized='view', schema='LANDING_CLEAN', alias='PORTAL_CKA_CALIFORNIA_OPEN_B100823F62') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.PORTAL_CKA_CALIFORNIA_OPEN_B100823F62, from the
...
select
    source.*,
    {{ landing_parse_date('"LIC_EXPIRATION_DATE"', 'us', 'TEXT') }} as lic_expiration_date_clean_ts,
    {{ landing_parse_date('"INGESTED_AT"', 'iso', 'TEXT') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'PORTAL_CKA_CALIFORNIA_OPEN_B100823F62') }} as source
