{{ config(materialized='view', schema='LANDING_CLEAN', alias='PORTAL_CKA_INDIANA_DATA_HUB_FE00D42ACC') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.PORTAL_CKA_INDIANA_DATA_HUB_FE00D42ACC, from the
...
select
    ...
    {{ landing_parse_date('"CURRENT_AS_OF"', 'iso', 'TEXT') }} as current_as_of_clean_ts,
    {{ landing_parse_date('"INGESTED_AT"', 'iso', 'TEXT') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'PORTAL_CKA_INDIANA_DATA_HUB_FE00D42ACC') }} as source
