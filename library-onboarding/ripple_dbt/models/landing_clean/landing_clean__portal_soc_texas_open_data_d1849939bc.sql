{{ config(materialized='view', schema='LANDING_CLEAN', alias='PORTAL_SOC_TEXAS_OPEN_DATA_D1849939BC') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.PORTAL_SOC_TEXAS_OPEN_DATA_D1849939BC, from the
...
select
    source.*,
    {{ landing_parse_date('"SIGNED_DATE"', 'iso', 'TEXT') }} as signed_date_clean_ts,
    {{ landing_parse_date('"INGESTED_AT"', 'iso', 'TEXT') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'PORTAL_SOC_TEXAS_OPEN_DATA_D1849939BC') }} as source
