{{ config(materialized='view', schema='LANDING_CLEAN', alias='PORTAL_SOC_UTAH_OPEN_DATA_P_D5F7CA2621') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.PORTAL_SOC_UTAH_OPEN_DATA_P_D5F7CA2621, from the
...
select
    source.*,
    {{ landing_parse_date('"C_32_DOC_CTRL_NUM"', 'epochms', 'TEXT') }} as c_32_doc_ctrl_num_clean_ts,
    {{ landing_parse_date('"INGESTED_AT"', 'iso', 'TEXT') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'PORTAL_SOC_UTAH_OPEN_DATA_P_D5F7CA2621') }} as source
