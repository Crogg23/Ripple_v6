{{ config(materialized='view', schema='LANDING_CLEAN', alias='PORTAL_CKA_ISRAEL_NATIONAL_4145FF0883') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.PORTAL_CKA_ISRAEL_NATIONAL_4145FF0883, from the
...
select
    source.*,
    {{ landing_parse_date('"VALIDUNTIL"', 'iso', 'TEXT') }} as validuntil_clean_ts,
    {{ landing_parse_date('"INGESTED_AT"', 'iso', 'TEXT') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'PORTAL_CKA_ISRAEL_NATIONAL_4145FF0883') }} as source
