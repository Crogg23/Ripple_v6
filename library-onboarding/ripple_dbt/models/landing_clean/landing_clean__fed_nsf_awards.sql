{{ config(materialized='view', schema='LANDING_CLEAN', alias='FED_NSF_AWARDS') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.FED_NSF_AWARDS, from the
...
select
    source.*,
    {{ landing_parse_date('"AWARD_DATE"', 'us', 'TEXT') }} as award_date_clean_ts,
    {{ landing_parse_date('"START_DATE"', 'us', 'TEXT') }} as start_date_clean_ts,
    {{ landing_parse_date('"END_DATE"', 'us', 'TEXT') }} as end_date_clean_ts,
    {{ landing_parse_audit_epoch('"_INGESTED_AT"') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'FED_NSF_AWARDS') }} as source
