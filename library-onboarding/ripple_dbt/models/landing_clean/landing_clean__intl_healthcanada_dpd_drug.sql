{{ config(materialized='view', schema='LANDING_CLEAN', alias='INTL_HEALTHCANADA_DPD_DRUG') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.INTL_HEALTHCANADA_DPD_DRUG, from the
...
select
    source.*,
    {{ landing_parse_date('"C_24_DEC_2025"', 'dmon', 'TEXT') }} as c_24_dec_2025_clean_ts,
    {{ landing_parse_audit_epoch('"INGESTED_AT"') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'INTL_HEALTHCANADA_DPD_DRUG') }} as source
