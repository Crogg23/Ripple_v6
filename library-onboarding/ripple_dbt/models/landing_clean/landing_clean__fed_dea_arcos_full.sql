{{ config(materialized='view', schema='LANDING_CLEAN', alias='FED_DEA_ARCOS_FULL') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.FED_DEA_ARCOS_FULL, from the
...
select
    source.*,
    {{ landing_parse_date('"TRANSACTION_DATE"', 'mdy8', 'TEXT') }} as transaction_date_clean_ts
from {{ source('ripple_raw', 'FED_DEA_ARCOS_FULL') }} as source
