{{ config(materialized='view', schema='LANDING_CLEAN', alias='FED_CFTC_COT_FINANCIAL') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.FED_CFTC_COT_FINANCIAL, from the
...
select
    source.*,
    {{ landing_parse_date('"REPORT_DATE_AS_YYYY_MM_DD"', 'iso', 'TEXT') }} as report_date_as_yyyy_mm_dd_clean_ts,
    {{ landing_parse_audit_epoch('"INGESTED_AT"') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'FED_CFTC_COT_FINANCIAL') }} as source
