{{ config(materialized='view', schema='LANDING_CLEAN', alias='FED_USGS_WBD_HUC8') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.FED_USGS_WBD_HUC8, from the
...
select
    source.*,
    {{ landing_parse_date('"LOADDATE"', 'epochms', 'FLOAT') }} as loaddate_clean_ts,
    {{ landing_parse_audit_epoch('"INGESTED_AT"') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'FED_USGS_WBD_HUC8') }} as source
