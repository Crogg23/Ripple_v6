{{ config(materialized='view', schema='LANDING_CLEAN', alias='ST_OEHHA_PROPOSITION_65_LIST') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.ST_OEHHA_PROPOSITION_65_LIST, from the
...
select
    source.*,
    {{ landing_parse_date('"DATE_LISTED"', 'iso', 'TEXT') }} as date_listed_clean_ts,
    {{ landing_parse_audit_epoch('"_INGESTED_AT"') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'ST_OEHHA_PROPOSITION_65_LIST') }} as source
