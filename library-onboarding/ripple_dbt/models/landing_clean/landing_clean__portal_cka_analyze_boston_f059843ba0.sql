{{ config(materialized='view', schema='LANDING_CLEAN', alias='PORTAL_CKA_ANALYZE_BOSTON_F059843BA0') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.PORTAL_CKA_ANALYZE_BOSTON_F059843BA0, from the
...
select
    source.*,
    {{ landing_parse_date('"INSTALLED_ON"', 'us', 'TEXT') }} as installed_on_clean_ts,
    {{ landing_parse_date('"INGESTED_AT"', 'iso', 'TEXT') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'PORTAL_CKA_ANALYZE_BOSTON_F059843BA0') }} as source
