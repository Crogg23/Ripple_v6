{{ config(materialized='view', schema='LANDING_CLEAN', alias='CA_LOBBY_CONTRIBUTIONS') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.CA_LOBBY_CONTRIBUTIONS, from the
...
select
    source.*,
    {{ landing_parse_date('"FILING_PERIOD_START_DT"', 'us', 'TEXT') }} as filing_period_start_dt_clean_ts,
    {{ landing_parse_date('"FILING_PERIOD_END_DT"', 'us', 'TEXT') }} as filing_period_end_dt_clean_ts,
    {{ landing_parse_audit_epoch('"INGESTED_AT"') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'CA_LOBBY_CONTRIBUTIONS') }} as source
