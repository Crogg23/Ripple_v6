{{ config(materialized='view', schema='LANDING_CLEAN', alias='FED_EPA_ICIS_AIR_ICIS_AIR_PROGRAMS') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.FED_EPA_ICIS_AIR_ICIS_AIR_PROGRAMS, from the
...
select
    source.*,
    {{ landing_parse_date('"BEGIN_DATE"', 'us', 'TEXT') }} as begin_date_clean_ts,
    {{ landing_parse_date('"UPDATED_DATE"', 'us', 'TEXT') }} as updated_date_clean_ts,
    {{ landing_parse_audit_epoch('"_INGESTED_AT"') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'FED_EPA_ICIS_AIR_ICIS_AIR_PROGRAMS') }} as source
