{{ config(materialized='view', schema='LANDING_CLEAN', alias='FED_GOOGLE_POLADS_ADVERTISER_WEEKLY_SPEND') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.FED_GOOGLE_POLADS_ADVERTISER_WEEKLY_SPEND, from the
...
select
    source.*,
    {{ landing_parse_date('"WEEK_START_DATE"', 'iso', 'TEXT') }} as week_start_date_clean_ts,
    {{ landing_parse_audit_epoch('"INGESTED_AT"') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'FED_GOOGLE_POLADS_ADVERTISER_WEEKLY_SPEND') }} as source
