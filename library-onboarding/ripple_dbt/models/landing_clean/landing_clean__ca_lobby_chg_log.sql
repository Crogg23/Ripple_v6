{{ config(materialized='view', schema='LANDING_CLEAN', alias='CA_LOBBY_CHG_LOG') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.CA_LOBBY_CHG_LOG, from the
...
select
    source.*,
    {{ landing_parse_date('"LOG_DT"', 'us', 'TEXT') }} as log_dt_clean_ts,
    {{ landing_parse_date('"ETHICS_DT"', 'us', 'TEXT') }} as ethics_dt_clean_ts,
    {{ landing_parse_date('"EFFECT_DT"', 'us', 'TEXT') }} as effect_dt_clean_ts,
    {{ landing_parse_audit_epoch('"INGESTED_AT"') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'CA_LOBBY_CHG_LOG') }} as source
