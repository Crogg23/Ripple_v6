{{ config(materialized='view', schema='LANDING_CLEAN', alias='PORTAL_SOC_OPEN_DATA_BR_FC0D2DAA2B') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.PORTAL_SOC_OPEN_DATA_BR_FC0D2DAA2B, from the
...
select
    source.*,
    {{ landing_parse_date('"MODIFIED_DATE"', 'iso', 'TEXT') }} as modified_date_clean_ts,
...
from {{ source('ripple_raw', 'PORTAL_SOC_OPEN_DATA_BR_FC0D2DAA2B') }} as source
