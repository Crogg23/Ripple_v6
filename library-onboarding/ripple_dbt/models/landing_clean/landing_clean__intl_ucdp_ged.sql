{{ config(materialized='view', schema='LANDING_CLEAN', alias='INTL_UCDP_GED') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.INTL_UCDP_GED, from the
...
select
    source.*,
    {{ landing_parse_date('"SOURCE_DATE"', 'iso', 'TEXT') }} as source_date_clean_ts,
...
from {{ source('ripple_raw', 'INTL_UCDP_GED') }} as source
