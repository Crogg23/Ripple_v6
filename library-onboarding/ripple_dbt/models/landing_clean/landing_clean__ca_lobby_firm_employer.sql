{{ config(materialized='view', schema='LANDING_CLEAN', alias='CA_LOBBY_FIRM_EMPLOYER') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.CA_LOBBY_FIRM_EMPLOYER, from the
...
select
    source.*,
    {{ landing_parse_date('"RPT_START"', 'us', 'TEXT') }} as rpt_start_clean_ts,
    {{ landing_parse_date('"RPT_END"', 'us', 'TEXT') }} as rpt_end_clean_ts,
    {{ landing_parse_audit_epoch('"INGESTED_AT"') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'CA_LOBBY_FIRM_EMPLOYER') }} as source
