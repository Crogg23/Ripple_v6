{{ config(materialized='view', schema='LANDING_CLEAN', alias='PORTAL_ARC_HARRIS_COUNTY_OP_CB850CC4B0') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.PORTAL_ARC_HARRIS_COUNTY_OP_CB850CC4B0, from the
...
select
    source.*,
    {{ landing_parse_date('"USER_SCH16"', 'us', 'TEXT') }} as user_sch16_clean_ts,
    {{ landing_parse_date('"INGESTED_AT"', 'iso', 'TEXT') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'PORTAL_ARC_HARRIS_COUNTY_OP_CB850CC4B0') }} as source
