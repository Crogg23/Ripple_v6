{{ config(materialized='view', schema='LANDING_CLEAN', alias='FED_EPA_TRI_BASIC_2023') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.FED_EPA_TRI_BASIC_2023, from the
...
select
    source.*,
    {{ landing_parse_date('"C_36_DOC_CTRL_NUM"', 'epochms', 'TEXT') }} as c_36_doc_ctrl_num_clean_ts,
    {{ landing_parse_audit_epoch('"INGESTED_AT"') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'FED_EPA_TRI_BASIC_2023') }} as source
