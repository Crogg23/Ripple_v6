{{ config(materialized='view', schema='LANDING_CLEAN', alias='FED_IRS_527_ORGS') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.FED_IRS_527_ORGS, from the
...
select
    source.*,
    {{ landing_parse_date('"F15"', 'ymd8', 'TEXT') }} as f15_clean_ts,
    {{ landing_parse_date('"F40"', 'ymd8', 'TEXT') }} as f40_clean_ts,
    {{ landing_parse_date('"FILED_DATE"', 'iso', 'TEXT') }} as filed_date_clean_ts,
    {{ landing_parse_audit_epoch('"_INGESTED_AT"') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'FED_IRS_527_ORGS') }} as source
