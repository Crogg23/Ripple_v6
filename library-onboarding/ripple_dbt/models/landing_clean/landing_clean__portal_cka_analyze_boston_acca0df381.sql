{{ config(materialized='view', schema='LANDING_CLEAN', alias='PORTAL_CKA_ANALYZE_BOSTON_ACCA0DF381') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.PORTAL_CKA_ANALYZE_BOSTON_ACCA0DF381, from the
...
select
    source.*,
    {{ landing_parse_date('"TIMESTAMP"', 'iso', 'TEXT') }} as timestamp_clean_ts,
    {{ landing_parse_date('"INGESTED_AT"', 'iso', 'TEXT') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'PORTAL_CKA_ANALYZE_BOSTON_ACCA0DF381') }} as source
