{{ config(materialized='view', schema='LANDING_CLEAN', alias='UK_COMPANIES_HOUSE_PSC') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.UK_COMPANIES_HOUSE_PSC, from the
...
select
    source.*,
    {{ landing_parse_date('"CEASED_ON"', 'iso', 'TEXT') }} as ceased_on_clean_ts,
    {{ landing_parse_date('"NOTIFIED_ON"', 'iso', 'TEXT') }} as notified_on_clean_ts,
    {{ landing_parse_audit_epoch('"_INGESTED_AT"') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'UK_COMPANIES_HOUSE_PSC') }} as source
