{{ config(materialized='view', schema='LANDING_CLEAN', alias='FED_OSHA_ITA_300A_SUMMARY_2023') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.FED_OSHA_ITA_300A_SUMMARY_2023, from the
...
select
    source.*,
    {{ landing_parse_date('"CREATED_TIMESTAMP"', 'us', 'TEXT') }} as created_timestamp_clean_ts,
    {{ landing_parse_audit_epoch('"INGESTED_AT"') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'FED_OSHA_ITA_300A_SUMMARY_2023') }} as source
