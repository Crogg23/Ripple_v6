{{ config(materialized='view', schema='LANDING_CLEAN', alias='INTL_FAO_FAOSTAT') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.INTL_FAO_FAOSTAT, from the
...
select
    source.*,
    {{ landing_parse_date('"DATEUPDATE"', 'iso', 'TEXT') }} as dateupdate_clean_ts,
    {{ landing_parse_audit_epoch('"_INGESTED_AT"') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'INTL_FAO_FAOSTAT') }} as source
