{{ config(materialized='view', schema='LANDING_CLEAN', alias='INTL_EU_SOCTA_EUROPOL') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.INTL_EU_SOCTA_EUROPOL, from the
...
select
    source.*,
    {{ landing_parse_date('"PUBLISH_DATE"', 'iso', 'TEXT') }} as publish_date_clean_ts,
...
from {{ source('ripple_raw', 'INTL_EU_SOCTA_EUROPOL') }} as source
