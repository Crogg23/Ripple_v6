{{ config(materialized='view', schema='LANDING_CLEAN', alias='ST_CA_OEHHA_PROP65_CHEMICALS') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.ST_CA_OEHHA_PROP65_CHEMICALS, from the
...
select
    source.*,
    {{ landing_parse_date('"UNNAMED_4"', 'dmon', 'TEXT') }} as unnamed_4_clean_ts,
    {{ landing_parse_audit_epoch('"INGESTED_AT"') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'ST_CA_OEHHA_PROP65_CHEMICALS') }} as source
