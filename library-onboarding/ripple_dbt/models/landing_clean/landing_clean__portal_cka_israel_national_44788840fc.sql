{{ config(materialized='view', schema='LANDING_CLEAN', alias='PORTAL_CKA_ISRAEL_NATIONAL_44788840FC') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.PORTAL_CKA_ISRAEL_NATIONAL_44788840FC, from the
...
select
    source.*,
    {{ landing_parse_date('"EQITF_CONFIRMATION_VALIDITY"', 'us', 'TEXT') }} as eqitf_confirmation_validity_clean_ts,
    {{ landing_parse_date('"INGESTED_AT"', 'iso', 'TEXT') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'PORTAL_CKA_ISRAEL_NATIONAL_44788840FC') }} as source
