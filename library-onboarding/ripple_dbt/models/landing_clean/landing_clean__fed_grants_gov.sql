{{ config(materialized='view', schema='LANDING_CLEAN', alias='FED_GRANTS_GOV') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.FED_GRANTS_GOV, from the
...
select
    source.*,
    {{ landing_parse_date('"POSTED_DATE"', 'us', 'TEXT') }} as posted_date_clean_ts,
...
from {{ source('ripple_raw', 'FED_GRANTS_GOV') }} as source
