{{ config(materialized='view', schema='LANDING_CLEAN', alias='PORTAL_ARC_OPEN_DATA_DC_F37F5DDC3D') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.PORTAL_ARC_OPEN_DATA_DC_F37F5DDC3D, from the
...
select
    source.*,
    {{ landing_parse_date('"DATA_VERSION"', 'ymd8', 'TEXT') }} as data_version_clean_ts,
    {{ landing_parse_date('"INGESTED_AT"', 'iso', 'TEXT') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'PORTAL_ARC_OPEN_DATA_DC_F37F5DDC3D') }} as source
