{{ config(materialized='view', schema='LANDING_CLEAN', alias='INTL_EURLEX_CELLAR') }}
...
--
-- Clean date/time columns for LIBRARY_RAW.LANDING.INTL_EURLEX_CELLAR, from the
...
select
    ...
    {{ landing_parse_date('"DATE_OF_DOCUMENT"', 'iso', 'TEXT') }} as date_of_document_clean_ts,
    {{ landing_parse_audit_epoch('"_INGESTED_AT"') }} as ingested_at_clean_ts
from {{ source('ripple_raw', 'INTL_EURLEX_CELLAR') }} as source
