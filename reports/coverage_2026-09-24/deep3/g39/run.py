"""g39 read-only runner. Usage: python run.py q01 [q02 ...]
Each qNN.sql in this folder: comment lines, then one SELECT/WITH statement.
Results -> qNN.csv; statement appended to ../g39.sql; counter in count.txt."""
import sys, re, csv, datetime, decimal
...
for lab, raw, body in stmts:
    if n >= 35:
        print("budget reached; not running", lab)
        break
    n += 1
    cnt_f.write_text(str(n))
    with open(HERE.parent / "g39.sql", "a", encoding="utf-8") as f:
        f.write(f"\n-- [{lab}] statement {n}\n{raw.strip().rstrip(';')};\n")
    try:
        cur.execute(body)
        cols = [d[0] for d in cur.description]
        rows = [[conv(v) for v in r] for r in cur.fetchall()]
        with open(HERE / f"{lab}.csv", "w", encoding="utf-8", newline="") as fh:
            w = csv.writer(fh)
            w.writerow(cols)
            w.writerows(rows)
