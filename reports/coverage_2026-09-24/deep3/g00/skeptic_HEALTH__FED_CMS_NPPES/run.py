"""Skeptic runner: read-only, one SELECT/WITH per block, tag deep3-skeptic-2026-09-24."""
import csv, re, sys
...
def blocks(text):
    parts = re.split(r"^(-- K\d+\b.*)$", text, flags=re.M)
    return [(parts[i].split()[1], parts[i], parts[i+1].strip().rstrip(";").strip()) for i in range(1, len(parts), 2)]
