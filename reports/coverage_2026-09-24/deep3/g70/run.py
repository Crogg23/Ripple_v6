"""g70 read-only runner. Usage: python run.py q01 [q02 ...]
Each qNN.sql in this folder: comment lines, then one SELECT/WITH statement.
Results -> qNN.json; statement appended to ../g70.sql; counter in count.txt."""
...
for lab, raw, body in stmts:
    ...
    with open(HERE.parent / "g70.sql", "a", encoding="utf-8") as f:
        f.write(f"\n-- [{lab}] statement {n}\n{raw.strip().rstrip(';')};\n")
    try:
        cur.execute(body)
