"""g68 deep3 runner: read-only SELECT/WITH only, logs every statement to g68.sql, saves results as pickled DataFrames."""
import sys, re, time
...
def main(qfile):
    ...
    for label, sql in qs:
        ...
        if err:
            print(f"== S{n:02d} {label} ({dt:.1f}s) ERR {err}")
            continue
        df = df0 if df0 is not None else pd.DataFrame(rows, columns=cols)
        print(f"== S{n:02d} {label} ({dt:.1f}s) {len(df)} rows")
