"""g62 read-only runner. Usage: python run.py q01 [q02 ...]
Each qNN.sql in this folder: comment lines, then one SELECT/WITH statement.
Results -> qNN.json; statement appended to ../g62.sql; counter in count.txt."""
...
if n + len(stmts) > BUDGET:
    raise SystemExit(f"refused: budget {BUDGET}, used {n}, asked {len(stmts)}")
c = db.connect()
cur = c.cursor()
...
for lab, raw, body in stmts:
    ...
    with open(HERE.parent / "g62.sql", "a", encoding="utf-8") as f:
        f.write(f"\n-- [{lab}] statement {n}\n{raw.strip().rstrip(';')};\n")
    try:
        cur.execute(body)
