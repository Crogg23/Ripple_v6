"""g52 runner: read-only statements, one connection per call.

Usage: python run.py batch.sql
The batch holds blocks headed by '-- Sxx title'. Each block is one SELECT/WITH.
Output: stdout (first 60 rows), g52/out_<Sxx>.txt, and g52/out_<Sxx>.csv (all rows).
"""
import csv
import re
import sys
    ...
    try:
        ...
        for label, head, body in todo:
            ...
            try:
                ...
                for r in rows[:(4 if '[pull]' in head else 60)]:
