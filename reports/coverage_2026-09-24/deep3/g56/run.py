"""g56 runner: read-only statements, one connection per call.

Usage: python run.py batch.sql
The batch holds blocks headed by '-- Sxx title'. Each block is one SELECT/WITH.
Output: stdout (first 40 rows), g56/out_<Sxx>.txt, and g56/out_<Sxx>.csv (all rows).
"""
import csv
import re
import sys
...
def main():
    ...
    for label, head, body in todo:
        ...
        if re.search(r"\b(CREATE|INSERT|UPDATE|DELETE|DROP|ALTER|MERGE|TRUNCATE|GRANT)\b", body, re.I):
            sys.exit(f"refusing {label}: write keyword")
    c = db.connect()
    try:
        cur = c.cursor()
