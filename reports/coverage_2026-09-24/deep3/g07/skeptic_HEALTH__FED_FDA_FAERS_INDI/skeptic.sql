-- s03_rederive
with t as (select coalesce(nullif(trim(ISR),''),nullif(trim(PRIMARYID),'')) id, trim(DRUG_SEQ) seq, upper(trim(ROLE_COD)) role,
...
d0 as (select coalesce(nullif(trim(ISR),''),nullif(trim(PRIMARYID),'')) id, coalesce(nullif(trim(C_CASE),''),nullif(trim(CASEID),'')) cs, SRC_QUARTER q,
...
ic as (select *, case when ip is null or ip like '%UNKNOWN%' then 'unknown'
  ...
  from i),
x as (select t.brand, t.role, d.id, d.cs, d.q, d.occ, d.snd, d.rept, d.mfr, d.evt, d.age, d.sex, d.ctry, ic.ip, ic.cls, ic.not_dx
...
oi as (select ids.cs, max(iff(regexp_like(upper(trim(n.INDI_PT)),'.*(CANCER|CARCINOMA|NEOPLASM|TUMOU?R|LYMPHOMA|LEUK|MYELOMA|METASTA|MALIGNAN|SARCOMA|MELANOMA|BLASTOMA|MESOTHELIOMA|ONCO|CHEMOTHERAPY|RADIOTHERAPY|PALLIATIVE|HOSPICE|HODGKIN|GLIOMA|MYELODYSPLAS).*'),1,0)) ind_ca
...
rr as (select ids.cs, max(iff(regexp_like(upper(trim(r.PT)),'.*(CANCER|CARCINOMA|NEOPLASM|TUMOU?R|LYMPHOMA|LEUK|MYELOMA|METASTA|MALIGNAN|SARCOMA|MELANOMA|BLASTOMA|MESOTHELIOMA|ONCO|CHEMOTHERAPY|RADIOTHERAPY|PALLIATIVE|HOSPICE|HODGKIN|GLIOMA|MYELODYSPLAS).*'),1,0)) reac_ca
...
dd as (select ids.cs, max(iff(regexp_like(upper(g.DRUGNAME),'.*(TAXOL|PACLITAXEL|DOCETAXEL|TAXOTERE|CISPLATIN|CARBOPLATIN|GEMCITABINE|GEMZAR|AVASTIN|BEVACIZUMAB|TARCEVA|ERLOTINIB|XELODA|CAPECITABINE|FLUOROURACIL|CYCLOPHOSPHAMIDE|DOXORUBICIN|TAMOXIFEN|ANASTROZOLE|ARIMIDEX|LETROZOLE|FEMARA|ZOMETA|XGEVA|RITUX|HERCEPTIN|GLEEVEC|GLIVEC|IRINOTECAN|OXALIPLATIN|ELOXATIN|ETOPOSIDE|VINCRISTINE|NEULASTA|PEGFILGRASTIM|FILGRASTIM|NEUPOGEN|ERBITUX|CETUXIMAB|ALIMTA|PEMETREXED|VELCADE|REVLIMID|THALOMID|TEMODAR|SUTENT|NEXAVAR|CASODEX|LUPRON|ZOLADEX|FASLODEX|AROMASIN|IRESSA|TYKERB|ABRAXANE|CAMPTOSAR|ADRIAMYCIN).*'),1,0)) onc_drug,
   max(iff(regexp_like(upper(g.DRUGNAME),'.*(PLACEBO|BLINDED|STUDY|INVESTIGATIONAL|UNBLIND).*'),1,0)) trial_drug
...
od as (select sid.cs, left(listagg(distinct upper(trim(g.DRUGNAME))||':'||upper(trim(g.ROLE_COD)), ' | '),500) drugs from LIBRARY_MARTS.HEALTH.HEALTH__FED_FDA_FAERS_DRUG g join sid on sid.id=coalesce(nullif(trim(g.ISR),''),nullif(trim(g.PRIMARYID),'')) group by 1),
oi as (select sid.cs, left(listagg(distinct upper(trim(n.INDI_PT)), ' | '),400) inds from LIBRARY_MARTS.HEALTH.HEALTH__FED_FDA_FAERS_INDI n join sid on sid.id=coalesce(nullif(trim(n.ISR),''),nullif(trim(n.PRIMARYID),'')) group by 1)
