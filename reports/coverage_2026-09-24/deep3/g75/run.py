"""g75 runner: read-only statements, one connection per call.

Usage: python run.py batch.sql
The batch holds blocks headed by '-- Sxx title'. Each block is one SELECT/WITH.
Output: stdout (first 60 rows), g75/out_<Sxx>.txt, and g00/out_<Sxx>.csv (all rows).
"""
import csv
import re
import sys
...
def main():
    ...
    try:
        ...
        for label, head, body in todo:
            ...
            with open(HERE / "count.txt", "a", encoding="utf-8") as fh:
