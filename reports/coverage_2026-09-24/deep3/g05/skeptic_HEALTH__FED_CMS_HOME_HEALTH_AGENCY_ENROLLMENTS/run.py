"""Skeptic read-only runner. python run.py s01 [s02 ...]. Each sNN.sql = one SELECT/WITH. Results -> sNN.json. Counter in count.txt (ALTERs counted too)."""
...
BAD = ["INS"+"ERT","UPD"+"ATE","DEL"+"ETE","DR"+"OP","CRE"+"ATE","MER"+"GE","TRUNC"+"ATE","GR"+"ANT","REV"+"OKE","CA"+"LL","PU"+"T","CO"+"PY"]
def clean(sql):
...
def conv(v):
    ...
    return v
labs = sys.argv[1:]
stm = [(l, clean((HERE/f"{l}.sql").read_text(encoding="utf-8"))) for l in labs]
cf = HERE/"count.txt"; n = int(cf.read_text()) if cf.exists() else 0
if n + 2 + len(stm) > 12: raise SystemExit(f"budget: {n} used, would be {n+2+len(stm)}")
