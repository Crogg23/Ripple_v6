"""g41 read-only runner. Usage: python run.py q01 [q02 ...]
Each qNN.sql in this folder: comment lines, then one SELECT/WITH statement.
Results -> qNN.json; statement appended to ../g41.sql; counter in count.txt."""
...
BAD = ["INS" + "ERT", "UPD" + "ATE", "DEL" + "ETE", "DR" + "OP", "CRE" + "ATE", "ALT" + "ER",
...
labels = sys.argv[1:]
stmts = [(lab, (HERE / f"{lab}.sql").read_text(encoding="utf-8")) for lab in labels]
stmts = [(lab, raw, clean(raw)) for lab, raw in stmts]
c = db.connect()
cur = c.cursor()
...
for lab, raw, body in stmts:
    ...
    with open(HERE.parent / "g41.sql", "a", encoding="utf-8") as f:
        f.write(f"\n-- [{lab}] statement {n}\n{raw.strip().rstrip(';')};\n")
    try:
        cur.execute(body)
