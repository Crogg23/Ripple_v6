"""g28 runner: one statement per call, read-only guard, logs SQL to g28.sql, output to g28/out_Sxx.pkl/.csv"""
import sys, re, time
...
import pandas as pd
HERE = REPO / "reports/coverage_2026-09-24/deep3/g28"
SQLLOG = REPO / "reports/coverage_2026-09-24/deep3/g28.sql"
...
def run(sid, label, sql, show=40):
    ...
    try:
        ...
        with open(SQLLOG, 'a', encoding='utf-8') as f:
            f.write("\n-- " + sid + " " + label + "  [FAILED, counted: " + str(e).splitlines()[0][:200] + " | qid " + str(qid) + "]\n" + s + ";\n")
        raise
