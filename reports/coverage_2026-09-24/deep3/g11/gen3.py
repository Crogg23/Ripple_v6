...
p65agg = f"""p65 AS (SELECT TRIM(cas_no) cas, MIN(chemical) p65_name,
       ...
       FROM {P65} WHERE cas_no NOT LIKE '--%' AND NULLIF(TRIM(cas_no),'') IS NOT NULL AND date_listed IS NOT NULL GROUP BY 1)"""
num = lambda c: f"TRY_TO_NUMBER(REPLACE({c},',',''))"
b = f"""-- S14 Prop 65 x TRI 2023 on CAS: land rate and pounds, split by TRI's own carcinogen flag (rerun of S12; total_releases is already FLOAT)
WITH {p65agg},
...
FROM {T}.HEALTH__INTL_HEALTHCANADA_DPD_DRUG GROUP BY 1 ORDER BY n DESC LIMIT 10;

-- S17 UDS: patients per weekly open site-hour, joined to the site roster on grant number, ranked inside each state
WITH c AS (SELECT bhcmisid, {num('T3A_L39_CA')} + {num('T3A_L39_CB')} tot FROM {T}.HEALTH__FED_HRSA_UDS_TABLE3A_PATIENTS),
i AS (SELECT bhcmisid, grantnumber, healthcentername, healthcentercity, healthcenterstate st, urbanruralflag ur FROM {T}.HEALTH__FED_HRSA_UDS_HEALTH_CENTER_INFO),
s AS (SELECT health_center_number g, COUNT(*) sites, SUM(TRY_TO_NUMBER(operating_hours_per_week::string)) hrs,
      ...
      FROM {T}.HEALTH__FED_HRSA_UDS_SERVICE_DELIVERY_SITES GROUP BY 1),
...
WITH s AS (SELECT raw:results[0]:identifiers[0]:id::string di0, raw:results[0]:identifiers[0]:type::string di0_type,
           ...
           FROM {T}.HEALTH__FED_FDA_GUDID__STAGING),
g AS (SELECT DISTINCT primary_di FROM {T}.HEALTH__FED_FDA_GUDID WHERE primary_di IN (SELECT di0 FROM s))
...
"""
open("batch3.sql","w",encoding="utf-8").write(b)
