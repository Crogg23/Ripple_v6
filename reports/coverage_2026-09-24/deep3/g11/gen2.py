lines = range(1, 40)
cols = ", ".join(f"T3A_L{i}_CA, T3A_L{i}_CB" for i in lines)
...
long_cte = f"""WITH t AS (SELECT bhcmisid, {cols} FROM {T}.HEALTH__FED_HRSA_UDS_TABLE3A_PATIENTS),
long AS (SELECT bhcmisid, col, val FROM t UNPIVOT(val FOR col IN ({cols}))),
...
p65agg = f"""p65 AS (SELECT TRIM(cas_no) cas, MIN(chemical) p65_name,
       ...
       FROM {P65} WHERE cas_no NOT LIKE '--%' AND NULLIF(TRIM(cas_no),'') IS NOT NULL GROUP BY 1)"""
b = f"""-- S08 GUDID staging: what the raw blocks hold (rerun of S04, which failed on the reserved word SAMPLE)
...
FROM {P65} GROUP BY 1 ORDER BY 1;
...
"""
open("batch2.sql","w",encoding="utf-8").write(b)
