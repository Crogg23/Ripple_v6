...
p65cas = f"""p65 AS (SELECT DISTINCT TRIM(cas_no) cas FROM {P65}
       WHERE cas_no NOT LIKE '--%' AND NULLIF(TRIM(cas_no),'') IS NOT NULL AND date_listed IS NOT NULL AND chemical NOT ILIKE '%delisted%')"""
b = f"""-- S25 TRI 2023 trap check: same site + same CAS on more than one row (rerun of S22, which failed on the reserved word SAMPLE)
...
WITH {p65cas}
SELECT COUNT(*) forms, COUNT(DISTINCT c_2_trifd) sites, ROUND(SUM({fug}+{stk})) air,
       ARRAY_SLICE(ARRAY_AGG(c_37_chemical||' '||ROUND({fug}+{stk})) WITHIN GROUP (ORDER BY {fug}+{stk} DESC),0,10) top_rows
FROM {TRI} t LEFT JOIN p65 ON p65.cas = TRIM(t.c_40_cas)
...
"""
open("batch6.sql","w",encoding="utf-8").write(b)
