"""g38 runner: read-only statements, one connection per call.

Usage: python run.py batch.sql
Blocks headed by '-- Sxx title'; each block is one SELECT/WITH.
Output: stdout (first 80 rows), g38/out_<Sxx>.txt and .csv (all rows).
"""
import csv
import re
import sys
