"""g20 deep3 runner: read-only SELECT/WITH only, logs every statement to g20.sql."""
...
SQL_LOG = HERE.parent / "g20.sql"
COUNT_FILE = HERE / "count.txt"
# statements this runner refuses to send (read-only brief)
REFUSE = ["INSERT", "UPDATE", "DELETE", "DROP", "CREATE", "ALTER", "MERGE", "TRUNCATE", "GRANT"]
...
def main(qfile):
    ...
    for label, sql in qs:
        ...
        if not err:
            print(" | ".join(cols))
            for r in rows[:int(__import__("os").environ.get("SHOW","40"))]:
