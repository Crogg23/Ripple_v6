"""g23 read-only runner. Usage: python run.py q01 [q02 ...]
Each qNN.sql in this folder: comment lines, then one SELECT/WITH statement.
Results -> qNN.json; statement appended to ../g23.sql; counters in count.txt (SELECT/WITH) and alter_count.txt."""
...
af = HERE / "alter_count.txt"
af.write_text(str((int(af.read_text()) if af.exists() else 0) + 2))
cnt_f = HERE / "count.txt"
n = int(cnt_f.read_text()) if cnt_f.exists() else 0
for lab, raw, body in stmts:
    n += 1
    cnt_f.write_text(str(n))
    with open(HERE.parent / "g23.sql", "a", encoding="utf-8") as f:
        f.write(f"\n-- [{lab}] statement {n}\n{raw.strip().rstrip(';')};\n")
    try:
        cur.execute(body)
