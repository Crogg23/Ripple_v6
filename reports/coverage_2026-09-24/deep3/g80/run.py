"""g80 runner: read-only statements, one connection per call.

Usage: python run.py batch.sql
The batch holds blocks headed by '-- Sxx title'. Each block is one SELECT/WITH.
Output: stdout (first 40 rows), g80/out_<Sxx>.txt, and g80/out_<Sxx>.csv (all rows).
"""
import csv
import re
import sys
...
def main():
    ...
    for label, head, body in todo:
        ...
        if BAD.search(code):
            sys.exit(f"refusing {label}: write keyword {BAD.search(code).group(0)}")
    c = db.connect()
    try:
        cur = c.cursor()
        ...
        for label, head, body in todo:
            ...
            try:
                ...
                for r in rows[:40]:
                    lines.append(" | ".join("" if v is None else str(v)[:200] for v in r))
