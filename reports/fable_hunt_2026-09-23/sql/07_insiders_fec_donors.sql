with ins as (
  select distinct r.OWNER_CIK, r.OWNER_NAME, r.STATE, r.RELATIONSHIP, r.TITLE, s.ISSUER_NAME, s.ISSUER_CIK,
...
fec as (
  select DONOR_NAME, EMPLOYER, OCCUPATION, STATE, CMTE_ID, TRANSACTION_AMT, TRANSACTION_DATE, OTHER_ID, CYCLE_FILE,
...
m as (
  select ins.OWNER_CIK, ins.OWNER_NAME, ins.ISSUER_NAME, ins.ISSUER_CIK, ins.RELATIONSHIP, ins.TITLE, fec.DONOR_NAME, fec.EMPLOYER, fec.STATE, fec.CMTE_ID, fec.OTHER_ID, fec.TRANSACTION_AMT, fec.TRANSACTION_DATE, fec.CYCLE_FILE
...
select count(*) rows_, count(distinct OWNER_CIK) insiders, count(distinct ISSUER_CIK) issuers, round(sum(TRANSACTION_AMT)/1e6,1) usd_m from m;
with ins as (
  select distinct r.OWNER_CIK, r.OWNER_NAME, r.STATE, r.RELATIONSHIP, r.TITLE, s.ISSUER_NAME, s.ISSUER_CIK,
    upper(split_part(trim(r.OWNER_NAME),' ',1)) last_nm, upper(regexp_replace(split_part(trim(r.OWNER_NAME),' ',2),'[^A-Za-z]','')) first_nm,
    upper(regexp_replace(split_part(upper(s.ISSUER_NAME),' ',1),'[^A-Z0-9]','')) issuer_w1
...
select m.OWNER_NAME, m.ISSUER_NAME, any_value(m.TITLE) title, count(*) gifts, round(sum(m.TRANSACTION_AMT)) usd, count(distinct m.CMTE_ID) committees,
  round(sum(iff(cm.pty='REP',m.TRANSACTION_AMT,0))) rep_usd, round(sum(iff(cm.pty='DEM',m.TRANSACTION_AMT,0))) dem_usd, max_by(cm.nm, m.TRANSACTION_AMT) biggest_recipient
from m left join cm on cm.CMTE_ID = m.CMTE_ID group by 1,2 order by usd desc limit 40
