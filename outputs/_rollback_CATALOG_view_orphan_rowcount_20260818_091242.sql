...
SELECT
    ...
    (COALESCE(lr.STATUS='success' AND (
     ...
     OR l.land_rows = 500000
     OR sn.SOURCE_ID IS NOT NULL) AS IS_SAMPLE,
    -- SAMPLE_NOTE / IS_DECLARED_SAMPLE (2026-08-11): the heuristics above INFER a
    -- truncated load from row counts. These two carry the mart author's OWN written
    -- statement that the table is a slice, lifted out of the model header into
    -- LIBRARY_META.REGISTRY.MART_SAMPLE_NOTES by scripts/build_sample_notes.py.
    -- Seventeen models said "SAMPLE ONLY -- NOT the full dataset" in plain words and
    -- nothing surfaced it: a chart builder saw the FULL source's name and a 'modeled'
    -- lifecycle. A declaration beats a heuristic, and the note says WHICH slice
    -- (HMDA is one state-year; FDIC BankFind is a 10,000-row API page), which is the
    -- part anyone drawing a conclusion actually needs.
    sn.SAMPLE_NOTE AS SAMPLE_NOTE,
    (sn.SOURCE_ID IS NOT NULL) AS IS_DECLARED_SAMPLE,
    CASE
...
LEFT JOIN staging s     ON s.sid  = i.sid
    LEFT JOIN LIBRARY_META.REGISTRY.MART_SAMPLE_NOTES sn ON UPPER(sn.SOURCE_ID) = UPPER(i.sid);
